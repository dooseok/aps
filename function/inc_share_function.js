		...
		for (cnt1 = 0; cnt1 < frmCommonList.strID_All.length; cnt1++)
		{
			if(arrColumn.length)
			{
				for(var cnt2 = 0; cnt2 < arrColumn.length; cnt2++)
				{
					...
					if(!strTemp.value)
					{
						strError += "* " + parseInt(cnt1+1) + "번째 항목 [" + arrName[cnt2] + "]은 필수항목 입니다.\n";
					}
					else if(arrType[cnt2] == "num" && !IsNum(strTemp.value))
					{
						strError += "* " + parseInt(cnt1+1) + "번째 항목 [" + arrName[cnt2] + "]은 숫자만 입력가능합니다.\n";
					}
					else if(arrType[cnt2].substring(0,3) == "fit")
					{
						...
						{
							strError += "* 항목 [" + arrName[cnt2] + "]은 "+ nLenLimit +"자리만 입력가능합니다.\n";
						}
					}
				}
			}
			else
			{
				...
				if(!strTemp.value)
				{
					strError += "* " + parseInt(cnt1+1) + "번째 항목 [" + strName + "]은 필수항목 입니다.\n";
				}
				else if(strType == "num" && !IsNum(strTemp.value))
				{
					strError += "* " + parseInt(cnt1+1) + "번째 항목 [" + strName + "]은 숫자만 입력가능합니다.\n";
				}
				else if(strType.substring(0,3) == "fit")
				{
					...
					if (strTemp.value.length != nLenLimit)
					{
						strError += "* 항목 [" + strName + "]은 "+ nLenLimit +"자리만 입력가능합니다.\n";
					}
				}
			}
		}
	...
	else
	{
		...
		if(arrColumn.length)
		{
			for(var cnt2 = 0; cnt2 < arrColumn.length; cnt2++)
			{
				...
				if(!strTemp.value)
				{
					strError += "* 항목 [" + arrName[cnt2] + "]은 필수항목 입니다.\n";
				}
				else if(arrType[cnt2] == "num" && !IsNum(strTemp.value))
				{
					strError += "* 항목 [" + arrName[cnt2] + "]은 숫자만 입력가능합니다.\n";
				}
				else if(arrType[cnt2].substring(0,3) == "fit")
				{
					var nLenLimit = 0;
		...
		else
		{
			...
			if(!strTemp.value)
			{
				strError += "* 항목 [" + strName + "]은 필수항목 입니다.\n";
			}
			else if(strType == "num" && !IsNum(strTemp.value))
			{
				strError += "* 항목 [" + strName + "]은 숫자만 입력가능합니다.\n";
			}
			else if(strType.substring(0,3) == "fit")
			{
				var nLenLimit = 0;
...
function IsNum(strValue)
{
	...
	for(var i=j ; i<strValue.length ; i++)
	{
...
function Calendar_Click(e) {
	cal_Day = e.title;
	if (cal_Day.length > 6) {
		if(location.href=="/mse_plan/inc_mp_view_list.asp")
		{
...
function Show_cal(sYear, sMonth, sDay) {
	...
	Cal_HTML += "</tr height=2px><td><img src='/img/blank.gif' width=1px height=2px></td></tr>";
	Cal_HTML += "<tr><td align=center colspan=7 STYLE='font-size:11;color:#333333'><span onclick='parent.fnChangeYearD("+NowThisYear+","+NowThisMonth+","+NowThisDay+");' style='cursor:hand'><u>오늘날짜로 이동</u></span></td>";
	Cal_HTML += "</tr>";
	...
	//행이 6개 행인지, 5개인지 구분
	if (intLoopWeek == 6)	calHeight = 239;
	else	calHeight = 214;
