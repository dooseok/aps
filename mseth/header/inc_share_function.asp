...
function getPWS_Raw_Data(PRD_Date,PRD_Line)
	...
	SQL = SQL & "	PRD_Line = 'pwsbox"&PRD_Line&"' "&vbcrlf
...
sub usePrinter()
%>
<OBJECT ID='factory' viewastext style='display:none' CLASSID='CLSID:1663ed61-23eb-11d2-b92f-008048fdd814' codebase='http://admin.spstest.com/function/smsx.cab#version=6,4,438,06'></OBJECT>
<%
end sub
%>
...
function show_WorkOrder_Guide(strObjectName, strExtraValue1, strExtraValue2)
{
	...
	divWorkOrder_Guide.style.posLeft	= event.x + 0 + document.body.scrollLeft;
	divWorkOrder_Guide.style.posTop		= event.y + 0 + document.body.scrollTop;
...
%>
	...
	{
		ifrmWorkOrderGuide.location.href	= "/function/inc_WorkOrder_guide.asp?BOM_Sub_BS_D_No=" + frmCommonListReg.BOM_Sub_BS_D_No.value;
	}
	else
	{
		strExtraValue2 = frmCommonList.strID_All.length;
		if (strExtraValue2 == 0)
			ifrmWorkOrderGuide.location.href	= "/function/inc_WorkOrder_guide.asp?BOM_Sub_BS_D_No=" + frmCommonList.BOM_Sub_BS_D_No.value;
		else
			ifrmWorkOrderGuide.location.href	= "/function/inc_WorkOrder_guide.asp?BOM_Sub_BS_D_No=" + frmCommonList.BOM_Sub_BS_D_No[strExtraValue1].value;
	}
<%
end if
%>
...
if instr(lcase(Request.ServerVariables("HTTP_URL")),"pr_list.asp") > 0 or instr(lcase(Request.ServerVariables("HTTP_URL")),"pr_plan_list.asp") > 0 then
%>
	ifrmWorkOrderGuide.location.href	= "about:blank";
<%
end if
%>
...
%>
	ifrmWorkOrderGuide.location.href	= "/function/inc_WorkOrder_guide.asp";
<%
end if
%>
}
...
function show_Parts_Guide(strObjectName, strExtraValue1, strExtraValue2)
{
	...
	divParts_Guide.style.posLeft	= event.x + 0 + document.body.scrollLeft;
	divParts_Guide.style.posTop		= event.y + 0 + document.body.scrollTop;
...
{
	ifrmPartsGuide.location.href	= "about:blank";
	objPartsGuide.value				= strValue;
	objPartsGuide.focus();
	hide_Parts_Guide();
	ifrmPartsGuide.location.href	= "/function/inc_parts_guide.asp";
}
</script>
...
function show_Material_Guide(strObjectName, strExtraValue1, strExtraValue2)
{
	...
	divMaterial_Guide.style.posLeft	= event.x + 0 + document.body.scrollLeft;
	divMaterial_Guide.style.posTop	= event.y + 0 + document.body.scrollTop;
...
{
	ifrmMaterialGuide.location.href	= "about:blank";
	objMaterialGuide.value			= strValue;
	objMaterialGuide.focus();
	hide_Material_Guide();
	ifrmMaterialGuide.location.href	= "/function/inc_Material_guide.asp";
<%
if instr(lcase(Request.ServerVariables("URL")),"/mod_list.asp") > 0 then
%>
	//ifrmPriceUpdate.location.href = "/material/order/detail/mod_price_updater.asp?objName="+objMaterialGuide+"&M_P_No=" + strValue + "&s_Partner_P_Name=<%=Request("s_Partner_P_Name")%>"
	var findedPrice = 0;
	
	var objRegMaterial_M_P_No	= eval("frmCommonListReg.Material_M_P_No");
	var objRegMOD_Price			= eval("frmCommonListReg.MOD_Price");
	var objListMaterial_M_P_No	= eval("frmCommonList." + objMaterialGuide.name);
	var objListMOD_Price		= eval("frmCommonList." + objMaterialGuide.name.replace('Material_M_P_No','MOD_Price'));
	
	//         Ʈ ѹ     ش   ϴ          迭     ã ´ .
	for(var i=0; i<arrPNO.length; i++){
		if(strValue==arrPNO[i])
			findedPrice = arrPrice[i];
	}

	if (findedPrice == 0)
	{
		for(var i=0; i<arrAlter_PNO.length; i++){
		if(strValue==arrAlter_PNO[i])
			findedPrice = arrAlter_Price[i]	;
		}
	}
	
	// Է          ׸ ȿ           Ʈ ѹ               ã                  Ʈ  Ѵ .
	if (objRegMaterial_M_P_No.length > 0) //       Է         
	{
		for(var i=0; i<objRegMaterial_M_P_No.length; i++)
		{
			if (strValue == objRegMaterial_M_P_No[i].value)
				objRegMOD_Price[i].value = findedPrice;
		}
	}
	else //       Է         .
	{
		if (objRegMaterial_M_P_No.value == strValue)
			objRegMOD_Price.value = findedPrice;
	}
	
	//    Ʈ      ׸ ȿ           Ʈ ѹ               ã                  Ʈ  Ѵ .
	for(var i=0; i<objListMaterial_M_P_No.length; i++)
	{
		if (objListMaterial_M_P_No[i].value == strValue)
			objListMOD_Price[i].value = findedPrice;
	}
<%
end if
%>
...
if instr(lcase(Request.ServerVariables("URL")),"/mod_list.asp") > 0 then
%>
<!--<iframe id="ifrmPriceUpdate" src="about:blank" width=600 height=100 frameborder=0></iframe>-->
var arrPNO			=new Array();
var arrPrice		=new Array();
var arrAlter_PNO	=new Array();
var arrAlter_Price	=new Array();
<%
dim RS1
dim SQL
dim CNT1
...
set RS1 = server.CreateObject("ADODB.RecordSet")
CNT1 = 0
SQL = "select MP_Price,Material_M_P_No from tbMaterial_Price where Partner_P_Name='"&Request("s_Partner_P_Name")&"'"
RS1.Open SQL,sys_DBCon
do until RS1.Eof
%>
arrPNO[<%=CNT1%>]	= '<%=RS1("Material_M_P_No")%>';
arrPrice[<%=CNT1%>] = '<%=RS1("MP_Price")%>';
<%
	CNT1 = CNT1 + 1
	RS1.MoveNext
loop
...
CNT1 = 0
SQL = "select M_P_No, M_Price from tbMaterial where M_Price is not null and convert(float,M_Price) > 0"
RS1.Open SQL,sys_DBCon
do until RS1.Eof
%>
arrAlter_PNO[<%=CNT1%>]	= '<%=RS1("M_P_No")%>';
arrAlter_Price[<%=CNT1%>] = '<%=RS1("M_Price")%>';
<%
	CNT1 = CNT1 + 1
	RS1.MoveNext
loop
...
<div id="divMaterial_Guide" style="width:50px;height:50px;position:absolute;display:none;border:1px solid #999999;">
...
function show_BOM_Guide(strObjectName, strExtraValue1, strExtraValue2)
{
	...
	divBOM_Guide.style.posLeft	= event.x + 0 + document.body.scrollLeft;
	divBOM_Guide.style.posTop	= event.y + 0 + document.body.scrollTop;
...
{
	ifrmBOMGuide.location.href	= "about:blank";
	objBOMGuide.value			= strValue;
	hide_BOM_Guide();
	ifrmBOMGuide.location.href	= "/function/inc_bom_guide.asp";
}
</script>
...
function show_BOMSub_Guide(strObjectName, strExtraValue1, strExtraValue2)
{
	...
	divBOMSub_Guide.style.posLeft	= event.x + 0 + document.body.scrollLeft;
	divBOMSub_Guide.style.posTop	= event.y + 0 + document.body.scrollTop;
...
{
	ifrmBOMSubGuide.location.href	= "about:blank";
	objBOMSubGuide.value			= strValue;
	hide_BOMSub_Guide();
	ifrmBOMSubGuide.location.href	= "/header/inc_bom_sub_guide.asp";
}
</script>
...
<tr>
	<td>
		<iframe id="ifrmBOMSubGuide" src="/header/inc_bom_sub_guide.asp" frameborder=0 width=250px height=286px></iframe>
	</td>
</tr>
</form>
</table>
...
%>
<script language="javascript">
function MTD_MoveFocus()
{
	if(event.keyCode == 13)
	{
		if (frmCommonListReg.Material_M_P_No.value.indexOf('--') != -1) //   ڵ       
		{
			strScan			= frmCommonListReg.Material_M_P_No.value; //        ڿ    strScan       
			var arrScan		= strScan.split('--');
			
			strIpgo_Date	= arrScan[1]; //  ¥     
			strMTD_Qty		= arrScan[2]; //           
			frmCommonListReg.Material_M_P_No.value = arrScan[0]; //  Ʈ ѹ      
			
			if (strIpgo_Date != '000000') //  ¥         ִٸ 
			{
				if (parseInt(Left(strIpgo_Date,2)) < 70)
					strIpgo_Date	= '20' + Left(strIpgo_Date,2) + '-' + strIpgo_Date.substr(2,2) + '-' + Right(strIpgo_Date,2) //  ¥    Ÿ     
				else
					strIpgo_Date	= '19' + Left(strIpgo_Date,2) + '-' + strIpgo_Date.substr(2,2) + '-' + Right(strIpgo_Date,2) //  ¥    Ÿ     

				frmCommonListReg.MTD_Ipgo_Date.value = strIpgo_Date; //  ¥  Է 
			}
			
			if (strMTD_Qty != '0') //             ִٸ 
				frmCommonListReg.MTD_Qty.value = strMTD_Qty; //      Է 
				
			
			if (strMTD_Qty == '0') //              ٸ 
				frmCommonListReg.MTD_Qty.focus(); //    ĭ      ̵ 
			else //            ִٸ 
				List_Reg(); //  Ͻõ 
		}
		else
		{
			frmCommonListReg.MTD_Qty.focus();
		}
	}
}
</script>
			<td><input type="text" size=10 name="<%=arrSelect(CNT1)%>" value="" onkeyup="javascript:MTD_MoveFocus();" onDblclick="javascript:show_Material_Guide(this);" class=input style="width:100%;text-align:<%=arrAlign(CNT1)%>;height:<%=strHeight-1%>px;padding-top:<%=nPaddingTop%>px;"></td>
<%
					else
%>
...
%>
			<td align="<%=arrAlign(CNT1)%>" title="<%=arrRecordSet(CNT1, CNT2)%>" style="padding-top:<%=nPaddingTop-2%>"><a href="/function/ifrm_download.asp?filepath=<%=arrDown(CNT1)%><%=arrRecordSet(CNT1, CNT2)%>" style='color:blue' target="ifrm_download"><%=arrRecordSet(CNT1, CNT2)%></a></td>
<%
...
<tr>
	<td width=100px align=left>
		...
		<tr>
			<td width=10px></td>
			<td width=40px align=right><img src="/img/blank.gif" width=1px height=2px><br>   μ &nbsp;</td>
			<td align=left>
				<select name="S_PageSize" onChange="frmPaging.submit();" style="width:42px">
<%
