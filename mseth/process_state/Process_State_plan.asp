...
'최적화 된 배열
dim arrOptBOM_Sub_BS_D_No(50)
dim arrOptPSP_Count(50)
dim arrOptPSP_ST(50)
dim arrOptPSP_Desc(50)
dim arrOptPSP_Start(50)
dim arrOptPSP_End(50)

dim arrOptPSP_Period(50)
...
'누적 시간 초기화 8시 20분이 자정으로 부터 30000분째
'nAccTime	= 30000
s_WorkStart = trim(Request("s_WorkStart"))
if s_WorkStart <> "" and len(s_WorkStart) = 4 and isNumeric(s_WorkStart) then
	nAccTime = int(left(s_WorkStart,2))*60*60 + int(right(s_WorkStart,2))*60
else
	nAccTime	= 28800
	s_WorkStart_H = int(nAccTime / 3600)
	if s_WorkStart_H < 10 then
		s_WorkStart_H = "0" & s_WorkStart_H
	end if
	
	s_WorkStart_M = int((nAccTime Mod 3600) / 60)
	if s_WorkStart_M < 10 then
		s_WorkStart_M = "0" & s_WorkStart_M
	end if
	
	s_WorkStart = s_WorkStart_H & s_WorkStart_M
end if
...
function save_frmPlan_State_list_to_db(form)
{
	if(form.lstBOM_Sub_BS_D_No.value == "" || form.lstPSP_Count.value == "")
	{
		alert("작업모델 및 목표수량을 입력해주세요.");
		return false;
	}
	else
	{
...
function save_frmPlan_State_to_db(form)
{
	form.s_WorkStart.value = frmPlan_State_list.s_WorkStart.value;
	form.submit();
}
...
<tr>
	<td>작업 시작시각 <input type="text" maxlength=4 size=4 value="<%=s_WorkStart%>" name="s_WorkStart"> (쉬는 시간을 피해서 입력해주세요.)</td>
</tr>
<tr height=400px>
	<td id="idPlan_State">
...
<tr height=22px>
	<td colspan=6><input type="button" value="현재목록 저장" onclick="javascript:save_frmPlan_State_list_to_db(this.form)"></td>
</tr>
</form>
</table>
...
<input type="hidden" value="<%=s_WorkStart%>" name="s_WorkStart">
<input type="hidden" name="s_Work_Date"			value="<%=request("s_Work_Date")%>">
<input type="hidden" name="s_Line"				value="<%=request("s_Line")%>">
<tr>
	<td><input type="button" value="현재목록 저장" onclick="javascript:save_frmPlan_State_to_db(this.form)"></td>
</tr>
<tr>
	<td id="idPlan_State">
...
%>
		<tr style="height:10px">
			...
			<!--<td>-->
				<input type='hidden'	name='PSP_Sub_YN' 		value="<%=CNT1%>">
				<input type='hidden'		name='PSP_Sub_Start'	value=""	maxlength=4 size=4>
				<input type='hidden'		name='PSP_Sub_End'		value=""	maxlength=4 size=4>
			<!--</td>-->
			<td>
