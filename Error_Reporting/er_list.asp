...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,부서,등록자,제목,등록일,수정일,작업"
strWidth			= ""
strWidth			= strWidth		& "40,50,60,250,80,80,50"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/error_reporting/ER_list.asp"
URL_View			= "/error_reporting/ER_edit_form.asp"
URL_Action			= "/error_reporting/ER_list_action.asp"
URL_Reg				= "/error_reporting/ER_reg_action.asp"

strTable			= "vwER_List"
strPK				= "ER_Code"
strSelect			= ""
strSelect			= strSelect		&"ER_Code,Member_M_Part,Member_M_Name,ER_Title,ER_Reg_Date,ER_Edit_Date"
...
'3/9
'----------------------------------------------------------------------------------
if Request("s_ER_title") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "ER_title like ''%"&Request("s_ER_title")&"%''"
end If
if Request("s_member_m_part") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "member_m_part like ''%"&Request("s_member_m_part")&"%''"
end If
if Request("s_member_m_name") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "member_m_name like ''%"&Request("s_member_m_name")&"%''"
end If

if S_Order_By_1 & S_Order_By_2 = "" then
	S_Order_By_1 	= "ER_code"
	S_Order_By_2 	= "desc"
end if
...
'4/9
'----------------------------------------------------------------------------------	
strColumn		= "s_member_m_part,s_member_m_name,s_ER_title"
strName			= "부서,등록자,제목"
strType			= BasicDataPart&",txt,txt"
'----------------------------------------------------------------------------------
...
%>
			<td width=5px></td>
			<td width=77px>
				<%=Make_BTN("신규등록","javascript:location.href='ER_reg_form.asp'","")%>
			</td>
			<td width=5px></td>
