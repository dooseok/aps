...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,사업부,파트넘버,마켓,입고일,판가,수량,입고금액,통화"
strWidth			= ""
strWidth			= strWidth		& "50,60,110,60,90,70,70,120,70"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/lg_receiving/lr_list.asp"
URL_View			= "/lg_receiving/lr_edit_form.asp"
URL_Action			= "/lg_receiving/lr_list_action.asp"
URL_Reg				= "/lg_receiving/lr_reg_action.asp"

strTable			= "tbLG_Receiving"
strPK				= "lr_code"
strSelect			= ""
strSelect			= strSelect		& "LR_Code,LR_Division,BOM_Sub_BS_D_No,LR_MKT,LR_Date,LR_Price,LR_Qty,LR_PriceQty,LR_Currency"
...
if Request("s_LR_Division") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "LR_Division like ''%"&Request("s_LR_Division")&"%''"
end If
if Len(Request("s_LR_Date")) = "22" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "LR_Date between ''"&Left(Request("s_LR_Date"),10)&"'' and ''"&Right(Request("s_LR_Date"),10)&"''"
end If
if S_Order_By_1 & S_Order_By_2 = "" then
	S_Order_By_1 	= "lr_code"
	S_Order_By_2 	= "desc"
end if
...
strColumn		= "s_bom_sub_bs_d_no,s_LR_Division,s_LR_Date"
strName			= "파트넘버,사업부,입고기간"
strType			= "txt,slt>DGZ:DGZ(RAC);DMZ:SAC(DMZ),dt2"
'----------------------------------------------------------------------------------
...
'6.5/9
'----------------------------------------------------------------------------------
SQL = "select sum(LR_PriceQty) from "&strTable&" "

if trim(strWhere) <> "" then
	SQL = SQL & " where " & strWhere
end if

call Common_Display_Summary("입고금액 총계 : |Sum_String|", SQL, "Y", strWidth_Total)
'----------------------------------------------------------------------------------
%>
