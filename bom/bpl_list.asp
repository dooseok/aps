...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "No,PartNo,Market,Currency,Price,Customer,Apply Date,Reg Date,Class,Tool,Desc,Spec,Del"
strWidth			= ""
strWidth			= strWidth		& "50,100,80,60,80,80,90,90,90,90,90,190,90,90,90"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/bom/bpl_list.asp"
URL_View			= "/bom/bpl_del_action.asp"
URL_Action			= "/bom/bpl_list_action.asp"
URL_Reg				= "/bom/bpl_reg_action.asp"

strTable			= "vwBPL_List"
strPK				= "BPL_Code"
strSelect			= ""
strSelect			= strSelect		& "BPL_Code,BOM_Sub_BS_D_No,BPL_Market,BPL_Currency,BPL_Price,Partner_P_Name,BPL_Apply_Date,BPL_Reg_Date,B_Class,B_Tool,B_Desc,B_Spec"
call Partner_Guide_Selling()
call BOMSub_Guide()
if Request("s_edit_mode_yn") = "" then
	strEdit = ",,,,,,,,,,,,,"
else
	strEdit = ",,"&basicDataMarket&","&basicDataCurrency&",txt,ptn,dt1,,,,,,"
end if
strPopup			= ",,,,,,,,,,,,,,,"
strDown				= ",,,,,,,,,,,,,,,"
...
if left(strSelectName,2) = "CHK" then
	Colspan	= Colspan + 1
end if
if right(strSelectName,2) = "Work" then
	Colspan	= Colspan + 1
end if
...
'6/9
'----------------------------------------------------------------------------------
	strReg	= ",dn2,"&basicDataMarket&","&basicDataCurrency&",txt,ptn,dt1,,,,,,"
'----------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Reg_Validater('BOM_Sub_BS_D_No,BPL_Market,BPL_Currency,BPL_Price,Partner_P_Name,BPL_Apply_Date','PartNo,Market,Currency,Price,Customer,Apply Date','txt,txt,txt,txt,txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Validater('BPL_Price,Partner_P_Name,BPL_Apply_Date','Price,Customer,Apply Date','txt,txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
...
%>
			<td width=5px></td>
			<td width=77px><%=Make_BTN("Submit","javascript:List_Update()","")%></td>
<%
end if
%>
<%
...
%>
			<td width=5px></td>
			<td width=77px>
				<div id="idBtnRegForm"><%=Make_BTN("Registration","javascript:RegForm_Toggle()","")%></div>
				<div id="idBtnList" style="display:none;"><%=Make_BTN("List","javascript:RegForm_Toggle()","")%></div>
			</td>
<%
end if
%>
			<td width=5px></td>
			<td width=77px><%=Make_BTN("down XLS","List2Excel()","")%></td>
			<td width=5px></td>
		</tr>
