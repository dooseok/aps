...
<tr>
	...
	<td width=410px>
		...
		<tr>
			<td width=55px>&nbsp;</td>
			<td width=200px align=right><input type="text" name="s_BOM_Sub_BS_D_No" value="<%=s_BOM_Sub_BS_D_No%>" readonly style="width:90px;text-align:center;font-size:20px;width:183px">&nbsp;</td>
			<td width=50px align=left><%=Make_S_BTN("조회","javascript:frmSearch.submit();","bom_editor")%></td>
			<td width=50px align=left><%=Make_S_BTN("인쇄","javascript:print();","bom_editor")%></td>
			<td width=55px>&nbsp;</td>
		</tr>
		</table>
...
<tr>
	<td>공정</td>
	<!--<td><input type="checkbox" name="s_P_Work_Type" readonly value="IMD"<%if instr(s_P_Work_Type,"IMD") > 0 then%> checked<%end if%>>IMD&nbsp;
		<input type="checkbox" name="s_P_Work_Type" readonly value="SMD"<%if instr(s_P_Work_Type,"SMD") > 0 then%> checked<%end if%>>SMD&nbsp;
		<input type="checkbox" name="s_P_Work_Type" readonly value="MAN"<%if instr(s_P_Work_Type,"MAN") > 0 then%> checked<%end if%>>MAN&nbsp;
		<input type="checkbox" name="s_P_Work_Type" readonly value="ASM"<%if instr(s_P_Work_Type,"ASM") > 0 then%> checked<%end if%>>ASM&nbsp;
		<input type="checkbox" name="s_P_Work_Type" readonly value="N/A"<%if instr(s_P_Work_Type,"N/A") > 0 then%> checked<%end if%>>N/A
	</td>-->
	...
	<td>수량</td>
	<td><input type="text" name="s_BQ_Qty" value="<%=s_BQ_Qty%>" readonly size=4 style="text-align:center;font-size:20px;"></td>
</tr>
</form>
</table>
<br>
...
SQL = "select BS_Code from tbBom_Sub where BS_D_No = '"&s_BOM_Sub_BS_D_No&"' and BOM_B_Code in (select B_Code from tbBOM where B_Version_Current_YN = 'Y')"
RS1.Open SQL,sys_DBCon
if not(RS1.Eof or RS1.Bof) then
...
<table width=1024px cellpadding=1 cellspacing=0 border=0 bgcolor="#ffffff" style="table-layout:fixed" style="border-collapse:collapse">
<tr>
	<td align=right style="font-size:15px"><b>엠에스이 주식회사&nbsp;&nbsp;</b></td>
</tr>
</table>
<%
