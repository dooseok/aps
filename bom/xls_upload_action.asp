...
if instr(temp,".xlsx") > 0 then
	XLSConnection = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" & temp & "; Extended Properties=""Excel 12.0;HDR=No;IMEX=1"""
elseif instr(temp,".xls") > 0 then
	XLSConnection = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & temp & "; Extended Properties=""Excel 8.0;HDR=No;IMEX=1"""
end if
...
If Err.Number <> 0 then
	...
	strError = "[파일업로드 실패] 다음의 내용을 확인해주십시오.\n1. 시트명은 [admin], [bomViewList.xls], [BOM Explosion] 등이 가능합니다\n2. 파일의 이름은 파트넘버와 일치하여야 합니다. 예)"&DNO&"01(...).xls"
%>
...
do until RS1.Eof
	if instr(RS1.Fields("table_name").Value,"_xlnm") = 0 then
		Sheet_Name = RS1(2)
	end if
	RS1.MoveNext
loop
...
if strError = "" then
	...
	'For i=0 to Ubound(arrXLS,2)
	'	For j=0 to Ubound(arrXLS,1)
	'		If arrXLS(j,i) <> "" Then
	'			Response.Write arrXLS(j,i)
	'			Response.Write "/"
	'		End If
	'	Next
	'	Response.Write "<br>"
...
<%		
			elseif CNT1 = 1 then
				...
				elseif instr(lcase(arrXLS(CNT2, CNT1)),"checksum") > 0 then
					CHECKSUM_LOC	= CNT2
				elseif instr(lcase(arrXLS(CNT2, CNT1)),"lg maker") > 0 then
					MAKER_LOC		= CNT2
...
<%
	...
	SQL = SQL & "	BQ_Order, "
	SQL = SQL & "	BQ_Qty "
	SQL = SQL & " from "&strTable&" where BOM_Sub_BS_D_No not in ("&strDNOSUB&") and BOM_B_Code = "&B_Code
