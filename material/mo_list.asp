...
strSelectName		= ""
strSelectName		= strSelectName & "번호,파트넘버,구분,스펙,BOM스펙,거래처,단가,확정여부,발주량,입고량,총미입고,발주자,발주일,납기일,발주완료,부서장,임원"
...
if Request("s_MO_Check_1_YN") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	if Request("s_MO_Check_1_YN") = "미결재" then
		strWhere = strWhere & "MO_Check_1_YN = ''''"
	elseif Request("s_MO_Check_1_YN") = "결재" then
		strWhere = strWhere & "MO_Check_1_YN <> ''''"
	end if
end If
...
if Request("s_MO_Check_2_YN") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	if Request("s_MO_Check_2_YN") = "미결재" then
		strWhere = strWhere & "MO_Check_2_YN = ''''"
	elseif Request("s_MO_Check_2_YN") = "결재" then
		strWhere = strWhere & "MO_Check_2_YN <> ''''"
	end if
end If
...
if Request("s_MO_Check_3_YN") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	if Request("s_MO_Check_3_YN") = "미결재" then
		strWhere = strWhere & "MO_Check_3_YN = ''''"
	elseif Request("s_MO_Check_3_YN") = "결재" then
		strWhere = strWhere & "MO_Check_3_YN <> ''''"
	end if
end If
...
'4/9
'----------------------------------------------------------------------------------
if instr("-구매-기획-총무-경영진-","-"&Request.Cookies("Admin")("M_Part")&"-") > 0  or instr("-shindk-kimdh-leehs-","-"&gM_ID&"-") > 0 then
	...
	strName			= "통합검색,파트넘버,거래처,발주자,발주기간|/|확인,부서장,임원,수정모드"
	strType			= "txt,txt,ptn,mnm-구매-자재-기획-,dt2|/|slt>:전체;결재:결재;미결재:미결재,slt>:전체;결재:결재;미결재:미결재,slt>:전체;결재:결재;미결재:미결재,chk"
else
	strColumn		= "s_Total,s_M_P_No,s_Partner_P_Name,s_MO_Reg_ID,s_MO_Due_Date|/|s_MO_Check_1_YN,s_MO_Check_2_YN,s_MO_Check_3_YN"
	strName			= "통합검색,파트넘버,거래처,발주자,발주기간|/|확인,부서장,임원"
	strType			= "txt,txt,ptn,mnm-구매-자재-기획-,dt2|/|slt>:전체;결재:결재;미결재:미결재,slt>:전체;결재:결재;미결재:미결재,slt>:전체;결재:결재;미결재:미결재"
end if
'----------------------------------------------------------------------------------
