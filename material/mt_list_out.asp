...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,파트넘버,구분,스펙,날짜,요청자,출고,직전재고,현재재고,비고,등록일,등록자"

strWidth			= ""
strWidth			= strWidth		& "40,100,150,200,70,100,60,70,70,200,140,70"
'----------------------------------------------------------------------------------
...
URL_This			= "/material/mt_list_out.asp"
URL_View			= "/material/mt_edit_form.asp"
URL_Action			= "/material/mt_list_out_action.asp"
URL_Reg				= "/material/mt_list_out_reg_action.asp"

strTable			= "vwMT_List_Out"
strPK				= "mt_code"
strSelect			= ""
strSelect			= strSelect		& "MT_Code,Material_M_P_No,M_Desc,M_Spec,MT_Date,MT_Out_byWho,MT_Qty_Out,MT_Qty_Last,MT_Qty_Now,MT_Desc,MT_Reg_Date,MT_Reg_ID"
...
if Request("s_Total") <> "" then
	...
	End If
	strWhere = strWhere & "(Material_M_P_No like ''%"&Request("s_Total")&"%'' or M_Desc like ''%"&Request("s_Total")&"%'' or M_Spec like ''%"&Request("s_Total")&"%'' or M_Spec like ''%"&Request("s_Total")&"%'' or MT_Out_byWho like ''%"&Request("s_Total")&"%'')"
end If
...
if Request("s_MT_Out_byWho") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "MT_Out_byWho like ''%"&Request("s_MT_Out_byWho")&"%''"
end If
...
if instr("-자재-기획-총무-경영진-","-"&Request.Cookies("Admin")("M_Part")&"-") > 0  or instr("-shindk-kimdh-leehs-","-"&gM_ID&"-") > 0 then
	strColumn		= "s_Total,s_Material_M_P_No,s_MT_Out_byWho,s_MT_Date,s_edit_mode_yn"
	strName			= "통합검색,파트넘버,요청자,날짜,수정모드"
	strType			= "txt,txt,txt,dt2,chk"
else
	strColumn		= "s_Total,s_Material_M_P_No,s_MT_Out_byWho,s_MT_Date"
	strName			= "통합검색,파트넘버,요청자,날짜"
	strType			= "txt,txt,txt,dt2"
end if
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------
	strReg	= ",mtr,,,dt1"&date()&",txt,num,,,txt,,,,,,,,,,,"
'----------------------------------------------------------------------------------
