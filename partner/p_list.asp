...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,상호,사업자번호,대표자,결재방식,메모,이메일주소"

strWidth			= ""
strWidth			= strWidth		& "40,150,100,100,100,200,100"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/partner/p_list.asp"
URL_View			= "/partner/p_edit_form.asp"
URL_Action			= "/partner/p_list_action.asp"
URL_Reg				= "/partner/p_reg_action.asp"

strTable			= "tbPartner"
strPK				= "p_code"
strSelect			= ""
strSelect			= strSelect		& "P_Code,P_Name,P_Business_No,P_Owner,P_Pay_Method,P_Memo,P_Email"

if Request("s_edit_mode_yn") = "" then
	strEdit = ",,,,,,"
else
	strEdit	= ",mem,mem,mem,"&BasicDataPartnerPaymentType&",mem,mem"
end if
strPopup			= ",,,,,,"
strDown				= ",,,,,,"
strAlign			= ""
strAlign			= strAlign		& "Center,Center,Center,Center,Center,Center,Center"
'----------------------------------------------------------------------------------

'3/9
'----------------------------------------------------------------------------------
if Request("s_P_Name") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "P_Name like ''%"&Request("s_P_Name")&"%''"
end If

if Request("s_P_Pay_Method") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "P_Pay_Method = ''"&Request("s_P_Pay_Method")&"''"
end If
...
'4/9
'----------------------------------------------------------------------------------
strColumn		= "s_P_Name,s_P_Pay_Method,s_edit_mode_yn"
strName			= "상호명,결재방식,수정모드"
strType			= "txt,"&BasicDataPartnerPaymentType&",chk"
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------
	strReg	= ",mem,mem,mem,"&BasicDataPartnerPaymentType&",mem,mem"
'----------------------------------------------------------------------------------
