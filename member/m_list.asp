...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,소속회사,아이디,암호,부서,직급,이름,이메일1,핸드폰,입사일,퇴사일,사용,권한"
strWidth			= ""
strWidth			= strWidth		& "50,90,90,90,90,90,90,150,100,90,90,60,100"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/member/m_list.asp"
URL_View			= "/member/m_delete_action.asp"
URL_Action			= "/member/m_list_action.asp"
URL_Reg				= "/member/m_reg_action.asp"

strTable			= "vwM_List"
strPK				= "M_Code"
strSelect			= ""
strSelect			= strSelect		& "M_Code,M_Channel,M_ID,M_Password,M_Part,M_Position,M_Name,M_Email_1,M_HP,M_Enter_Date,M_Retire_Date,M_Use_YN,M_Authority"

if Request("s_edit_mode_yn") = "" then
	strEdit = ",,,,,,,,,,,,"
else
	strEdit	= ","&BasicDataChannel&",txt,txt,"&BasicDataPart&","&BasicDataPosition&",txt,mem,mem,dt1,dt1,slt>Y:사용;N:미사용,mem"
end if
strPopup			= ",,,,,,,,,,,,"
strDown				= ",,,,,,,,,,,,"
...
'3/9
'----------------------------------------------------------------------------------
if Request("s_m_channel") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "m_channel = ''"&Request("s_m_channel")&"''"
end If
if Request("s_m_id") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "m_id like ''%"&Request("s_m_id")&"%''"
end If
if Request("s_m_part") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "m_part like ''%"&Request("s_m_part")&"%''"
end If
if Request("s_m_position") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "m_position like ''%"&Request("s_m_position")&"%''"
end If
if Request("s_m_name") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "m_name like ''%"&Request("s_m_name")&"%''"
end If
if Request("s_m_email") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "m_email_1 like ''%"&Request("s_m_email")&"%''"
end If
if Request("s_m_hp") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "m_hp like ''%"&Request("s_m_hp")&"%''"
end If
...
'4/9
'----------------------------------------------------------------------------------
strColumn		= "s_m_id,s_m_name,s_m_email,s_hp_1|/|s_m_channel,s_m_part,s_m_position,s_edit_mode_yn"
strName			= "아이디,이름,이메일,핸드폰|/|소속회사,부서,직급,수정모드"
strType			= "txt,txt,txt,txt|/|"&BasicDataChannel&","&BasicDataPart&","&BasicDataPosition&",chk"
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------
	strReg	= ","&BasicDataChannel&",txt,txt,"&BasicDataPart&","&BasicDataPosition&",txt,mem,mem,dt1,,slt>Y:사용;N:미사용,mem"
'----------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Reg_Validater('M_Channel,M_ID,M_Password,M_Part,M_Position,M_Name','소속회사,아이디,암호,부서,직급,이름','txt,txt,txt,txt,txt,txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
