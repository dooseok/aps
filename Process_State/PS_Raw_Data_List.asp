...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,바코드,파트넘버,라인,ICT날짜,ICT시간,포장날짜,포장시간"
strWidth			= ""
strWidth			= strWidth		& "60,160,90,90,70,60,70,60"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/process_state/ps_raw_data_list.asp"
URL_View			= "/process_state/ps_raw_data_list.asp"
URL_Action			= "/process_state/ps_raw_data_list.asp"
URL_Reg				= "/process_state/ps_raw_data_list.asp"

strTable			= "tbPWS_Raw_Data"
strPK				= "PRD_BarCode"
strSelect			= "PRD_Code,PRD_BarCode,PRD_PartNo,PRD_Line,PRD_ICT_Date,PRD_ICT_Time,PRD_Box_Date,PRD_Box_Time"
...
'3/9
'----------------------------------------------------------------------------------
if Request("s_PRD_BarCode") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PRD_BarCode = ''"&Request("s_PRD_BarCode")&"''"
end If
...
if Request("s_PRD_Line") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PRD_Line = ''"&Request("s_PRD_Line")&"''"
end If
if Len(Request("s_PRD_ICT_Date")) = "22" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PRD_ICT_Date between ''"&Left(Request("s_PRD_ICT_Date"),10)&"'' and ''"&Right(Request("s_PRD_ICT_Date"),10)&"''"
end if

if S_Order_By_1 & S_Order_By_2 = "" then
	S_Order_By_1 	= "PRD_Box_Date"
	S_Order_By_2 	= "desc"
end if
if S_Order_By_3 & S_Order_By_4 = "" then
	S_Order_By_3 	= "PRD_Box_Time"
	S_Order_By_4 	= "desc"
end if
...
'----------------------------------------------------------------------------------
call BOM_Guide()
strColumn		= "s_PRD_BarCode,s_PRD_PartNo|/|s_PRD_Line,s_PRD_ICT_Date"
strName			= "바코드,파트넘버|/|라인,ICT날짜"
strType			= "txt,txt|/|slt>pwsbox1:pwsbox1;pwsbox2:pwsbox2;pwsbox3:pwsbox3;pwsbox4:pwsbox4;pwsbox51:pwsbox51;pwsbox52:pwsbox52;pwsboxcell:pwsboxcell,dt2"
'----------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Validater('B_Standard_Time','ST','num');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
