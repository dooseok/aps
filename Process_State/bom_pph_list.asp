...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,파트넘버,시간당생산량,삭제"

strWidth			= ""
strWidth			= strWidth		& "100,120,120,120"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/process_state/bom_pph_list.asp"
URL_View			= "/process_state/bom_pph_del_action.asp"
URL_Action			= "/process_state/bom_pph_list_action.asp"
URL_Reg				= "/process_state/bom_pph_reg_action.asp"

strTable			= "tbBOM_PPH"
strPK				= "bp_code"
strSelect			= ""
strSelect			= strSelect		& "BP_Code,BOM_Sub_BS_D_No,BP_PPH"
...
if Request("s_pph_zero") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "bp_pph <= 0"
end If
...
strColumn		= "s_bom_sub_bs_d_no,s_pph_zero,s_edit_mode_yn"
strName			= "파트넘버,생산량미등록,수정모드"
strType			= "txt,chk,chk"
'----------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
%>
	var strError_1	= "BOM_Sub_BS_D_No,BP_PPH";
	var strError_2	= "파트넘버,시간당생산량";
...
%>
	...
	var strError_1	= "BP_PPH";
	var strError_2	= "시간당생산량";
	var strError_3	= "num";
...
function frmFileUpload_Check()
{
	...
	{
		Show_Progress();
		if(confirm("1.엑셀 2003 형식(.xls)로 저장되어야 합니다.\n2.새로운 파트넘버는 신규등록되며,\n  중복된 파트넘버는 덮어쓰기 됩니다.\n3.형식) B열 파트넘버, C열에 생산량,\n  (A열은 아무값이나 있어도 됩니다. 단 빈판은 안됨)"))
...
<tr>
	<td align=center>
		...
		<form name="frmFileUpload" action="bom_pph_xls_upload_action.asp" method="post" enctype="MULTIPART/FORM-DATA">
		<tr>
			<td align=center>
				<Table cellpadding=0 cellspacing=0 border=0>
				<tr>
...
%>
		...
		<tr>
			<td height=5px><img src="/img/blank.gif" width=1px height=5px></td>	
		</tr>
		<tr>
			<td align=center>
				<Table cellpadding=0 cellspacing=0 border=0>
				<tr>
					...
					<td width=5px></td>
					<td width=77px><%=Make_BTN("XLS UP","frmFileUpload_Check()","")%></td>
					<td width=5px></td>
					<td width=77px><%=Make_BTN("XLS DOWN","List2Excel()","")%></td>
					<td width=5px></td>
				</tr>
				</table>
			</td>
