...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,날짜,파트넘버,재고량,변동량,구분,비고,거래처,가격,작업자"

strWidth			= ""
strWidth			= strWidth		& "40,100,100,70,70,100,300,120,80,100"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/parts_transaction/pt_list.asp"
URL_View			= "/parts_transaction/pt_edit_form.asp"
URL_Action			= "/parts_transaction/pt_list_action.asp"
URL_Reg				= "/parts_transaction/pt_reg_action.asp"

strTable			= "vwPT_list"
strPK				= "pt_code"
strSelect			= ""
strSelect			= strSelect		& "PT_Code,PT_Date,Parts_P_P_No,Parts_P_Qty,PT_Qty,PT_Type,PT_Description,Partner_P_Name,PT_Price,Member_M_ID"

if Request("s_edit_mode_yn") = "" then
	strEdit = ",,,,,,,,,,,,,,,,,,,,,"
else
	strEdit	= ",mem,"&BasicDataPartnerSort&",mem,mem,mem,"&BasicDataPartnerPaymentType&",mem,mem,mem,mem,mem,mem,mem,mem,mem,mem,mem,mem,mem,mem"
end if
strPopup			= ",,,,,,,,,"
strDown				= ",,,,,,,,,"
strAlign			= "Center,Center,Center,Right,Right,Center,Center,Center,Right,Center"
'----------------------------------------------------------------------------------
...
if Request("s_pt_type") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "pt_type = ''"&Request("s_pt_type")&"''"
end If


if Len(Request("s_PT_Date2")) = "22" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PT_Date between ''"&Left(Request("s_PT_Date2"),10)&"'' and ''"&Right(Request("s_PT_Date2"),10)&"''"
else
	if Request("s_PT_Date") <> "" then
		If Trim(strWhere) <> "" Then
			strWhere = strWhere & " and "
		End If
		strWhere = strWhere & "PT_Date = ''"&Request("s_PT_Date")&"''"
	end If
end If


if S_Order_By_1 & S_Order_By_2 = "" then
	S_Order_By_1 	= "pt_code"
	S_Order_By_2 	= "desc"
end if
...
'4/9
'----------------------------------------------------------------------------------
strColumn		= "s_parts_p_p_no,s_pt_type|/|s_pt_date,s_pt_date2"
strName			= "파트넘버,작업구분|/|날짜,기간"
strType			= "txt,"&BasicDataPartsTransactionType&"|/|dt1,dt2"
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------
	strReg	= ",dt1"&date()&",pno,,num,,mem,"&"slt>:미정;"&replace(strEdit_Partner,"slt>","")&",mny,"
'----------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
%>
	var strError_1	= "Parts_P_P_No,PT_Qty";
...
<tr>
	<td align=center>
		...
		<tr>
<%
if Request("s_edit_mode_yn") <> "" and instr(admin_partner_p_list,"-"&gM_ID&"-") > 0 then
%>
			<td width=5px></td>
...
%>
<%
if Reg_Form_YN = "Y" and instr(admin_partner_p_list,"-"&gM_ID&"-") > 0 then
%>
			<td width=5px></td>
			<td width=77px>
