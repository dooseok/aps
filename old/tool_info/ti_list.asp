...
'----------------------------------------------------------------------------------
strSelectName		= ""
if instr(admin_ti_list,"-"&Request("Admin")("M_ID")&"-") > 0 then
	strSelectName		= strSelectName & "번호,툴,구분,삭제"
else
	strSelectName		= strSelectName & "번호,툴,구분"
end if
strWidth			= ""
strWidth			= strWidth		& "100,100,150,100"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/tool_info/ti_list.asp"
URL_View			= "/tool_info/ti_delete_action.asp"
URL_Action			= "/tool_info/ti_list_action.asp"
URL_Reg				= "/tool_info/ti_reg_action.asp"

strTable			= "tbTool_Info"
strPK				= "TI_Code"
strSelect			= ""
strSelect			= strSelect		& "TI_Code,TI_Name,TI_Type"

if Request("s_edit_mode_yn") = "" then
	strEdit = ",,,"
else
	strEdit	= ",txt,"&BasicDataTool&","
end if
strPopup			= ",,,"
strDown				= ",,,"
strAlign			= "Center,Center,Center,Center"
'----------------------------------------------------------------------------------

'3/9
'----------------------------------------------------------------------------------

if Request("s_TI_Name") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "TI_Name like ''"&Request("s_TI_Name")&"''"
end If
if Request("s_TI_Type") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "TI_Type = ''"&Request("s_TI_Type")&"''"
end If

if S_Order_By_1 & S_Order_By_2 = "" then
	S_Order_By_1 	= "ti_code"
	S_Order_By_2 	= "desc"
end if
...
'4/9
'----------------------------------------------------------------------------------	
strColumn		= "s_TI_Name,s_TI_Type,s_edit_mode_yn"
strName			= "툴이름,툴구분,수정모드"
strType			= "txt,"&BasicDataTool&",chk"
'----------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Reg_Validater('TI_Name,TI_Type','툴,구분','txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Validater('TI_Name,TI_Type','툴,구분','txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
...
<tr>
	<td align=center>
		...
		<tr>
<%
if Request("s_edit_mode_yn") <> "" and instr(admin_ti_list,"-"&Request("Admin")("M_ID")&"-") > 0 then
%>
			<td width=5px></td>
...
%>
<%
if Reg_Form_YN = "Y" and instr(admin_ti_list,"-"&Request("Admin")("M_ID")&"-") > 0 then
%>		
			<td width=5px></td>
			<td width=77px>
