...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,발주일,납기일,진행상태,거래처,항목수,발주금액,입고금액,작업"
strWidth			= ""
strWidth			= strWidth		& "60,120,120,120,120,70,90,90,60"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/parts/order/po_list.asp"
URL_View			= "/parts/order/po_edit_form.asp"
URL_Action			= "/parts/order/po_list_action.asp"
URL_Reg				= "/parts/order/po_reg_action.asp"

strTable			= "vwParts_Order"
strPK				= "PO_Code"
strSelect			= ""
strSelect			= strSelect		& "PO_Code,PO_Date,PO_Due_Date,PO_State,Partner_P_Name,cntPOD,Total_Price,Total_In_Price"
...
if Request("s_edit_mode_yn") = "" then
	...
	strEdit	= ",dt1,dt1,"&BasicDataMaterialOrderState&",,,,,"
end if
strPopup			= ",,,,,,,,"
strDown				= ",,,,,,,,"
strAlign			= "Center,Center,Center,Center,Center,Center,right,right"
'----------------------------------------------------------------------------------

'3/9
'----------------------------------------------------------------------------------

if Len(Request("s_PO_date")) = "22" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PO_date between ''"&Left(Request("s_PO_date"),10)&"'' and ''"&Right(Request("s_PO_date"),10)&"''"
end if

if Len(Request("s_PO_due_date")) = "22" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PO_due_date between ''"&Left(Request("s_PO_due_date"),10)&"'' and ''"&Right(Request("s_PO_due_date"),10)&"''"
end if
...
if Request("s_Parts_p_p_no") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PO_Code in (select Parts_Order_PO_Code from tbParts_Order_Detail where Parts_p_p_no = ''"&Request("s_Parts_p_p_no")&"'')"
end If
...
'4/9
'----------------------------------------------------------------------------------
strColumn		= "s_PO_date,s_PO_state,s_partner_p_name|/|s_po_due_date,s_parts_p_p_no,s_edit_mode_yn"
strName			= "발주일,상태,거래처|/|납기일,파트넘버,수정모드"
strType			= "dt2,"&BasicDataMaterialOrderState&",txt|/|dt2,mtr,chk"
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------
	strReg	= ",dt1"&date()&",dt1,"&BasicDataMaterialOrderState&","&strEdit_Partner&",,,,"
'----------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Reg_Validater('PO_Date,PO_Due_Date,PO_State','발주일,납기일,진행상태','txt,txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Validater('PO_Date,PO_Due_Date,PO_State','발주일,납기일,진행상태','txt,txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
