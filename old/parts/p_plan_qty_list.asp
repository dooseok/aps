...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "체크,번호,파트넘버,파트이름,스펙,4주간소요량,현재재고,1주후재고,2주후재고,3주후재고,4주후재고"

strWidth			= ""
strWidth			= strWidth		& "30,40,100,150,300,80,80,80,80,80,80,50"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/parts/p_plan_qty_list.asp"
URL_View			= "/parts/p_plan_qty_edit_form.asp"
URL_Action			= "/parts/p_plan_qty_list_action.asp"
URL_Reg				= "/parts/p_plan_qty_reg_action.asp"

strTable			= "vwP_Plan_Qty_List"
strPK				= "p_code"
strSelect			= ""
strSelect			= strSelect		& "P_Code,P_P_No,P_Desc,P_Spec,SUM_BQ_Qty,P_Qty,P_Incoming_Qty_1,P_Incoming_Qty_2,P_Incoming_Qty_3,P_Incoming_Qty_4"
...
strDown				= ",,,,,,,,,,,,"
strAlign			= ""
strAlign			= strAlign		& "Center,Center,Center,Center,Right,Right,Right,"
strAlign			= strAlign		& "Right,Right,Right,Right,Center,Center"
'----------------------------------------------------------------------------------
...
if Request("s_parts_p_p_no") <> "" then
	...
	End If
	strWhere = strWhere & "P_P_No like ''%"&Request("s_parts_p_p_no")&"%''"
end If

if Request("s_1w_under") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "SUM_BQ_Qty > P_Incoming_Qty_1"
end If

if Request("s_2w_under") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "SUM_BQ_Qty > P_Incoming_Qty_2"
end If

if Request("s_3w_under") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "SUM_BQ_Qty > P_Incoming_Qty_3"
end If

if Request("s_4w_under") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "SUM_BQ_Qty > P_Incoming_Qty_4"
end If
...
'----------------------------------------------------------------------------------	
call Parts_Guide
strColumn		= "s_parts_p_p_no,s_1w_under,s_2w_under,s_3w_under,s_4w_under"
strName			= "부품파트넘버,1주후미달,2주후미달,3주후미달,4주후미달"
strType			= "pno,chk,chk,chk,chk"
'----------------------------------------------------------------------------------
...
<tr>
	<td align=center>
		...
		<tr>
<%
if Request("s_edit_mode_yn") <> "" and instr(admin_p_plan_qty_list,"-"&Request("Admin")("M_ID")&"-") > 0  then
%>
			<td width=5px></td>
