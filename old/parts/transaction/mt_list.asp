...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,날짜,구분,거래처,비고,작업"
strWidth			= ""
strWidth			= strWidth		& "60,120,120,80,200,60"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/material/transaction/mt_list.asp"
URL_View			= "/material/transaction/mt_edit_form.asp"
URL_Action			= "/material/transaction/mt_list_action.asp"
URL_Reg				= "/material/transaction/mt_reg_action.asp"

strTable			= "tbMaterial_Transaction"
strPK				= "MT_Code"
strSelect			= ""
strSelect			= strSelect		& "MT_Code,MT_Date,MT_State,MT_Company,MT_Remark"

if Request("s_edit_mode_yn") = "" then
	strEdit = ",,,,,"
else
	strEdit	= ",dt1,,"&BasicDataMaterialTransactionCompany&",txt,"
end if
strPopup			= ",,,,,"
strDown				= ",,,,,"
...
if Request("s_mt_state") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "mt_state = ''"&Request("s_mt_state")&"''"
end If

if Request("s_mt_company") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "mt_company = ''"&Request("s_mt_company")&"''"
end If
...
'4/9
'----------------------------------------------------------------------------------
strColumn		= "s_mt_date,s_mt_state,s_mt_company,s_edit_mode_yn"
strName			= "날짜,구분,거래처,수정모드"
strType			= "dt2,"&BasicDataMaterialTransactionState&","&BasicDataMaterialTransactionCompany&",chk"
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------
	strReg	= ",dt1"&date()&","&BasicDataMaterialTransactionState&","&BasicDataMaterialTransactionCompany&"/d엠에스이,txt,"
'----------------------------------------------------------------------------------
