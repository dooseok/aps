...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,파트넘버,공정,종류,스펙,부가설명,재고수량,미입고량,작업"
strWidth			= ""
strWidth			= strWidth		& "70,150,80,150,250,300,70,70,50"
'----------------------------------------------------------------------------------
...
strPK				= "M_Code"
strSelect			= ""
strSelect			= strSelect		& "M_Code,M_P_No,M_Process,M_Desc,M_Spec,M_Additional_Info,M_Qty,Not_In_Qty"

if Request("s_edit_mode_yn") = "" then
	strEdit = ",,,,,,,,,,,,"
else
	strEdit = ",txt,"&BasicDataMaterialProcess&",txt,txt,txt,num,,"
end if
strPopup			= ",,,,,,/material/stock_history/msh_list.asp,"
strDown				= ",,,,,,,"
strAlign			= "Center,Center,Center,Center,left,left,right,right,Center,"
'----------------------------------------------------------------------------------

'3/9
'----------------------------------------------------------------------------------
if Request("s_M_Code") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "M_Code = ''"&Request("s_M_Code")&"''"
end If
...
if Request("s_M_Spec") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "M_Spec like ''%"&Request("s_M_Spec")&"%''"
end If
if Request("s_M_Additional_Info") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "M_Additional_Info like ''%"&Request("s_M_Additional_Info")&"%''"
end If
if Request("s_M_Qty_Up") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "M_Qty >= "&Request("s_M_Qty_Up")
end If
if Request("s_M_Qty_Down") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "M_Qty <= "&Request("s_M_Qty_Down")
end If
if Request("s_Partner_P_Name") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "M_P_No in (select Material_M_P_No from tbMaterial_Price where Partner_P_Name like ''%"&Request("s_Partner_P_Name")&"%'')"
end If
if Request("s_M_Price_Up") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "M_P_No in (select Material_M_P_No from tbMaterial_Price where MP_Price >= "&Request("s_M_Price_Up")&")"
end If
if Request("s_M_Price_Down") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "M_P_No in (select Material_M_P_No from tbMaterial_Price where MP_Price <= "&Request("s_M_Price_Up")&")"
...
'4/9
'----------------------------------------------------------------------------------
strColumn		= "s_m_p_no,s_m_spec,s_m_additional_info,s_partner_p_name|/|s_m_qty_up,s_m_qty_down,s_m_price_up,s_m_price_down,s_edit_mode_yn"
strName			= "파트넘버,스펙,설명,거래처|/|수량(이상),수량(이하),단가(이상),단가(이하),수정모드"
strType			= "mtr,txt,txt,txt|/|num,num,num,num,chk"
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------
	strReg	= ",txt,"&BasicDataMaterialProcess&",txt,txt,txt,num,"
'----------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Reg_Validater('M_P_No','파트넘버','txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Validater('M_P_No','파트넘버','txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
