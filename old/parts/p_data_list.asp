...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "체크,번호,파트넘버,공정,파트이름,현재재고,안전재고,LG가,단가1,거래처1,단가2,거래처2"

strWidth			= ""
strWidth			= strWidth		& "30,40,100,60,120,65,65,75,75,120,75,120,50"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/parts/p_data_list.asp"
URL_View			= "/parts/p_data_edit_form.asp"
URL_Action			= "/parts/p_data_list_action.asp"
URL_Reg				= "/parts/p_data_reg_action.asp"

strTable			= "vwP_List"
strPK				= "p_code"
strSelect			= ""
strSelect			= strSelect		& "P_Code,P_P_No,P_Work_Type,P_Desc,P_Qty,P_Safe_Qty,P_LGE_Price,Partner_P_Price_1,Partner_P_Name_1,Partner_P_Price_2,Partner_P_Name_2"
...
if Request("s_edit_mode_yn") = "" then
	...
	strEdit	= ",mem,"&BasicDataPartsType&",mem,mem,mem,mny,mny,"&"slt>:미정;"&replace(strEdit_Partner,"slt>","")&",mny,"&"slt>:미정;"&replace(strEdit_Partner,"slt>","")&","
...
strAlign			= strAlign		& "right,Center,Right,Center,Center,Center"
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------	
	strReg	= ",mem,"&BasicDataPartsType&",mem,mem,mem,mny,mny,"&"slt>:미정;"&replace(strEdit_Partner,"slt>","")&",mny,"&"slt>:미정;"&replace(strEdit_Partner,"slt>","")&","
'----------------------------------------------------------------------------------	
...
'6.5/9
'----------------------------------------------------------------------------------
SQL = "select sum(P_Qty*Partner_P_Price_1) from "&strTable&" "
...
<tr>
	<td align=center>
		...
		<tr>
<%
if Request("s_edit_mode_yn") <> "" and instr(admin_p_data_list,"-"&Request("Admin")("M_ID")&"-") > 0 then
%>
			<td width=5px></td>
...
%>
<%
if Reg_Form_YN = "Y" and instr(admin_p_data_list,"-"&Request("Admin")("M_ID")&"-") > 0 then
%>		
			<td width=5px></td>
			<td width=77px>
