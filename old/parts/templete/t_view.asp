...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,파트넘버,공정,스펙,부가설명,수량,삭제"
strWidth			= ""
strWidth			= strWidth		& "70,150,80,200,200,160,70,70,50"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/Parts/templete/t_list.asp"
URL_View			= "/Parts/templete/t_delete_action.asp"
URL_Action			= "/Parts/templete/t_list_action.asp"
URL_Reg				= "/Parts/templete/t_reg_action.asp"

strTable			= "vwParts_Templete_Detail"
strPK				= "MTD_Code"
strSelect			= ""
strSelect			= strSelect		& "MTD_Code,Parts_M_P_No,Parts_M_Process,Parts_M_Spec,Parts_M_Additional_Info,MTD_Count"

if Request("s_edit_mode_yn") = "" then
	strEdit = ",,,,,,,,,"
else
	strEdit = ",mtr,"&BasicDataPartsProcess&",,,num,"
end if
strPopup			= ",,,,,,,"
strDown				= ",,,,,,,"
...
if Request("s_M_Price_Up") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "M_Price >= "&Request("s_M_Price_Up")
end If
if Request("s_M_Price_Down") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "M_Price <= "&Request("s_M_Price_Down")
end If
...
'4/9
'----------------------------------------------------------------------------------
strColumn		= "s_m_p_no,s_m_spec,s_m_additional_info,s_parnter_p_name|/|s_m_qty_up,s_m_qty_down,s_m_price_up,s_m_price_down,s_edit_mode_yn"
strName			= "파트넘버,스펙,설명,거래처|/|수량(이상),수량(이하),단가(이상),단가(이하),수정모드"
strType			= "txt,txt,txt,txt|/|num,num,num,num,chk"
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------
	strReg	= ",mtr,"&BasicDataPartsProcess&",,,num,"
'----------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Reg_Validater('Parts_M_P_No,MTD_Count','파트넘버,수량','txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Validater('Parts_M_P_No,MTD_Count','파트넘버,수량','txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
