...
strWidth			= ""
strWidth			= strWidth		& "70,150,80,200,200,70,70"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "mtd_list.asp"
URL_View			= "mtd_delete_action.asp"
URL_Action			= "mtd_list_action.asp"
URL_Reg				= "mtd_reg_action.asp"

strTable			= "vwMaterial_Templete_Detail"
strPK				= "MTD_Code"
strSelect			= ""
strSelect			= strSelect		& "MTD_Code,Material_M_P_No,Material_M_Process,Material_M_Spec,Material_M_Additional_Info,MTD_Count"
...
if Request("s_edit_mode_yn") = "" then
	...
	strEdit = ",mtr,"&BasicDataMaterialProcess&",,,num,"
end if
strPopup			= ",,,,,,,"
strDown				= ",,,,,,,"
...
'3/9
'----------------------------------------------------------------------------------
if Request("s_MT_Name") = "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "Material_Templete_MT_Name = ''none_data''"
end If
if Request("s_MT_Name") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "Material_Templete_MT_Name = ''"&Request("s_MT_Name")&"''"
end If
...
'4/9
'----------------------------------------------------------------------------------
strColumn		= "s_MT_Name,s_m_p_no,s_m_spec,s_m_additional_info,s_edit_mode_yn"
strName			= "템플릿,파트넘버,스펙,설명,수정모드"
strType			= "txt,txt,txt,txt,chk"
'----------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Reg_Validater('Material_M_P_No,MTD_Count','파트넘버,수량','txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Validater('Material_M_P_No,MTD_Count','파트넘버,수량','txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
