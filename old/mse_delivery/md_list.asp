...
if instr(admin_lm_list,"-"&Request("Admin")("M_ID")&"-") > 0 then
	strSelectName		= strSelectName & "번호,납품일,라인,파트넘버,워크오더,입고워크오더,납품량,WO납품,WO입고,WO차이,PN납품,PN입고,PN차이,납품처,비고,삭제"
else
	strSelectName		= strSelectName & "번호,납품일,라인,파트넘버,워크오더,입고워크오더,납품량,WO납품,WO입고,WO차이,PN납품,PN입고,PN차이,납품처,비고"
end if
strWidth			= ""
strWidth			= strWidth		& "70,100,80,120,130,130,50,50,50,50,50,50,50,50,200,60"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/mse_delivery/md_list.asp"
URL_View			= "/mse_delivery/md_delete_action.asp"
URL_Action		= "/mse_delivery/md_list_action.asp"
URL_Reg				= "/mse_delivery/md_reg_action.asp"

strTable			= "vwMSE_Delivery"
strPK					= "MD_Code"
strSelect			= ""
strSelect			= strSelect		& "MD_Code,MD_Date,MD_Line,BOM_Sub_BS_D_No,MD_Work_Order,MD_Work_Order_Ipgo,MD_Qty,WO_Qty,WO_Ipgo_Qty,WO_Diff,PN_Qty,PN_Ipgo_Qty,PN_Diff,MD_Dest,MD_Remark"
...
if Request("s_edit_mode_yn") = "" then
	...
	strEdit	= ",dt1,txt,txt,dn2,txt,txt,txt,txt,txt,txt,txt,txt,txt,txt,txt,txt,txt"
end if
strPopup			= ",,,/product_delivery/pd_list.asp?s_BOM_Sub_BS_D_No=,/product_delivery/pd_list.asp?s_PD_Work_Order=,/product_delivery/pd_list.asp?s_PD_Work_Order=,,,,,,,,,,,,"
strDown				= ",,,,,,,,,,,,,,,,,,"
strAlign			= "Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center"
'----------------------------------------------------------------------------------
...
if S_Order_By_1 & S_Order_By_2 = "" then
	S_Order_By_1 	= "md_date"
	S_Order_By_2 	= "desc"
	S_Order_By_3 	= "md_code"
	S_Order_By_4 	= "desc"
end if
...
'4/9
'----------------------------------------------------------------------------------	
strColumn		= "s_BOM_Sub_BS_D_No,s_MD_Date,s_MD_Work_Order_Ipgo|/|s_MD_Work_Order,s_MD_Dest,s_edit_mode_yn"
strName			= "파트넘버,납품일,납품처|/|워크오더,입고워크오더,수정모드"
strType			= "dn2,dt2,txt|/|txt,txt,chk"
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------	
	strReg	= ",dt1,txt,txt,dn2,txt,txt,txt,txt,txt,txt,txt,txt,txt,txt,txt,txt,txt"
'----------------------------------------------------------------------------------	
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Reg_Validater('LM_Company,LM_Name','업체구분,모델명','txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Validater('LM_Company,LM_Name','업체구분,모델명','txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
...
{
	if(confirm("미분류 상태의 모델을 전부 타사로 전환하겠습니까?"))
	{
...
%>
			<!--<td width=5px></td>
			<td width=77px><%=Make_BTN("타사전환","javascript:Change_To_Other()","")%></td>-->
<%
end if
%>
