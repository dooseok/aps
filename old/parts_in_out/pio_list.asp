...
strSelectName		= strSelectName & "번호,부서,입출고 구분,날짜,상태,작업"
strWidth			= ""
strWidth			= strWidth		& "80,100,100,100,100,80"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/parts_in_out/pio_list.asp"
URL_View			= "/parts_in_out/pio_delete_action.asp"
URL_Action			= "/parts_in_out/pio_list_action.asp"
URL_Reg				= "/parts_in_out/pio_reg_action.asp"

strTable			= "tbParts_In_Out"
strPK				= "PIO_Code"
strSelect			= ""
strSelect			= strSelect		& "PIO_Code,PIO_Part,PIO_Type,PIO_Date,PIO_State"
...
if Request("s_PIO_Part") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PIO_Part = ''"&BasicDataPart&"''"
end If

if Len(Request("s_PIO_Date")) = "22" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PIO_Date between ''"&Left(Request("s_PIO_Date"),10)&"'' and ''"&Right(Request("s_PIO_Date"),10)&"''"
end if

if Request("s_PIO_Type") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PIO_Type = ''"&Request("s_PIO_Type")&"''"
end If

if S_Order_By_1 & S_Order_By_2 = "" then
	S_Order_By_1 	= "PIO_Code"
	S_Order_By_2 	= "desc"
end if
...
'4/9
'----------------------------------------------------------------------------------	
strColumn		= "s_PIO_Part,s_PIO_Type,s_PIO_Date"
strName			= "부서,입출고 구분, 날짜"
strType			= BasicDataPart&",slt>출고:출고;입고:입고,dt2"
'----------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Reg_Validater('PIO_Type','입출고 구분','txt,txt,txt,txt,txt,txt,txt,txt');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
