...
<form name="frmDate_Search" action="lp_view_summary_parts.asp" method="post">
<tr height=25px>
	<td bgcolor=white>
...
<br>
<table width="<%=400+(30*(s_Diff_LPD_Input_Date+1))%>"px cellpadding=0 cellspacing=0 border=0 bgcolor="#999999" class="LGE_Plan">
<tr bgcolor=dimgray>
	<td width=100px style="color:white"><b>PART NO</b></td>
	<td width=100px style="color:white"><b>TOOL</b></td>
	<td width=100px style="color:white"><b>PCB</b></td>
	<td width=100px style="color:white"><b>MICOM</b></td>
<%	
for CNT1 = 0 to s_Diff_LPD_Input_Date
	strDate = dateadd("d",CNT1,s_Min_LPD_Input_Date)
	strDate = right(strDate,2)
%>
	<td width=30px style="color:white"><b><%=strDate%></b></td>
<%
next
%>
</td>
...
do until RS1.Eof
	...
	for CNT1=0 to ubound(arrBOM_Sub_BS_D_No)
		...
		strPCB		= ""
		strMICOM	= ""
		SQL = 		"select top 2 "&vbcrlf
		SQL = SQL & "	Parts = Parts_P_P_No "&vbcrlf
		SQL = SQL & "from "&vbcrlf
		SQL = SQL & "	tbBOM_Qty "&vbcrlf
		SQL = SQL & "	inner join "&vbcrlf
		SQL = SQL & "	tbBOM_Sub "&vbcrlf
		SQL = SQL & "	on BOM_Sub_BS_D_No = BS_D_No and BS_D_No = '"&arrBOM_Sub_BS_D_No(CNT1)&"' "&vbcrlf
		SQL = SQL & "	inner join "&vbcrlf
		SQL = SQL & "	tbBOM "&vbcrlf
		SQL = SQL & "	on B_Code = BOM_B_Code and B_Current_YN = 'Y' "&vbcrlf
		SQL = SQL & "where "&vbcrlf
		SQL = SQL & "	BQ_Qty > 0 and ("&vbcrlf
		SQL = SQL & "	(left(Parts_P_P_No,4) = '6870' or left(Parts_P_P_No,3) = 'eax') or "&vbcrlf
		SQL = SQL & "	(left(Parts_P_P_No,4) in ('0IGS','0IHI','0IMC','0INE') or left(Parts_P_P_No,6) in ('0IMI38','0ITO87','EAN305','EAN309','EAN310','EAN323','EAN334','EAN348','EAN356','EAN360','EAN366','EAN369','EAN420','EAN430')) "&vbcrlf
		SQL = SQL & "	) "&vbcrlf
		RS2.Open SQL,sys_DBCon
		if RS2.Eof or RS2.Bof then
		else
			do until RS2.Eof
				if instr(lcase(RS2("Parts")),"6870") > 0 or instr(lcase(RS2("Parts")),"eax") > 0 then
					strPCB		= RS2("Parts")
				else
					strMICOM	= RS2("Parts")
				end if
...
<tr bgcolor=white height=20px>
	...
	<td width=100px><%=strPCB%></td>
	<td width=100px><%=strMICOM%></td>
<%	
...
%>
	<td width=30px bgcolor="<%=strBG_Color%>"><%=strQty%></td>
<%
		next
%>
</tr>
