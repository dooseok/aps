...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,날짜,부서,거래선,품명,용도(대분류),용도(소분류),금액,결재<br><img src='/img/blank.gif' width=1px height=5px><br>팀장-이사-사장,비고,삭제"
strWidth			= ""
strWidth			= strWidth		& "70,80,80,100,150,100,150,80,100,100,50"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/cost_report/cr_list.asp"
URL_View			= "/cost_report/cr_delete_action.asp"
URL_Action			= "/cost_report/cr_list_action.asp"
URL_Reg				= "/cost_report/cr_reg_action.asp"

strTable			= "vwCost_Report"
strPK				= "CR_Code"
strSelect			= ""
strSelect			= strSelect		& "CR_Code,CR_Date,CR_Part,CR_Line,CR_Title,CR_Use1,CR_Use2,CR_Amount,strCR_Sign,CR_Memo"

if Request("s_edit_mode_yn") = "" then
	strEdit = ",,,,,,,,,,"
else
	strEdit = ",dt1"&date()&","&BasicDataPart&",txt,txt,"&BasicDataCostReportUse1&",txt,txt,,txt,,,,"
end if
strPopup			= ",,,,,,,,,,"
strDown				= ",,,,,,,,,,"
...
'3/9
'----------------------------------------------------------------------------------
if Request("s_cr_line") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "CR_Line like ''%"&Request("s_cr_line")&"%''"
end If
if Request("s_cr_part") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "CR_Part = ''"&Request("s_cr_part")&"''"
end If
if Request("s_cr_title") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "Cr_Title like ''%"&Request("s_cr_title")&"%''"
end If
if Request("s_cr_use1") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "Cr_Use1 = ''"&Request("s_cr_use1")&"''"
end If
if Request("s_cr_use2") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "Cr_Use2 like ''%"&Request("s_cr_use2")&"%''"
end If


if S_Order_By_1 & S_Order_By_2 = "" then
	S_Order_By_1 	= "cr_code"
	S_Order_By_2 	= "desc"
end if
...
'4/9
'----------------------------------------------------------------------------------
strColumn		= "s_cr_date1,s_cr_part,s_cr_line,s_cr_title|/|s_cr_date2,s_cr_use1,s_cr_use2,s_edit_mode_yn"
strName			= "날짜,부서,거래선,품명|/|기간,용도(대),용도(소),수정모드"
strType			= "dt1,"&BasicDataPart&",txt,txt|/|dt2,"&BasicDataCostReportUse1&",txt,chk"
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------
	strReg	= ",dt1"&date()&","&BasicDataPart&",txt,txt,"&BasicDataCostReportUse1&",txt,txt,,txt,,,,"
'----------------------------------------------------------------------------------
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Reg_Validater('CR_Part,CR_Title,CR_Use1,CR_Amount','부서,품명,용도(대분류),금액','txt,txt,txt,num');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
...
'----------------------------------------------------------------------------------
%>
	var strError = List_Validater('CR_Part,CR_Title,CR_Use1,CR_Amount','부서,품명,용도(대분류),금액','txt,txt,txt,num');
<%
'----------------------------------------------------------------------------------
%>
	if(!strError)
