...
arrPI_Issued_Date		= split(Request("PI_Issued_Date")	,", ")
...
for CNT1 = 0 to ubound(arrID_All)
	...
	arrPI_Issued_Date(CNT1)		= trim(arrPI_Issued_Date(CNT1))
	...
	for CNT1 = 0 to ubound(arrID_All)
		...
		if strError_Temp = "" then
			if arrParts_P_P_No(CNT1) <> "" then
				...
				RS1.Open "tbParts_Incoming",sys_DBConString,3,2,2
				with RS1
					.AddNew
					...
					if arrPI_Issued_Date(CNT1) <> "" then
						.Fields("PI_Issued_Date")	= arrPI_Issued_Date(CNT1)
					end if
				...
				'신규 발주 항목에, 거래처와 단가가 입력되어 있다면.
				if arrPartner_P_Name(CNT1) <> "" and IsNumeric(arrPI_Price(CNT1)) then
					
					'해당 파트넘버에 대한 단가정보가 있는지.
					SQL = "select PP_Code from tbParts_Price where Parts_P_P_No = '"&arrParts_P_P_No(CNT1)&"'"
					...
					'해당 파트넘버에 대한, 해당 거래처의 단가정보가 있는지.
					SQL = "select PP_Code from tbParts_Price where Parts_P_P_No = '"&arrParts_P_P_No(CNT1)&"' and Partner_P_Name='"&arrPartner_P_Name(CNT1)&"'"
					RS1.Open SQL,sys_DBCon
					if RS1.Eof or RS1.Bof then
						Exist_Partner_YN = "N"	
					else
