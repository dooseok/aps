...
if instr(admin_pi_list,"-"&Request("Admin")("M_ID")&"-") > 0 then
	strSelectName		= strSelectName & "체크,번호,발주일,파트넘버,납기일,입고일,거래처,단가,수량,총계,비고,상태,거래구분,삭제"
else
	strSelectName		= strSelectName & "체크,번호,발주일,파트넘버,납기일,입고일,거래처,단가,수량,총계,비고,상태,거래구분"
end if
strWidth			= ""
strWidth			= strWidth		& "30,60,100,100,100,100,100,80,60,100,100,100,100,60"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/parts_incoming/pi_list.asp"
URL_View			= "/parts_incoming/pi_delete_action.asp"
URL_Action			= "/parts_incoming/pi_list_action.asp"
URL_Reg				= "/parts_incoming/pi_reg_action.asp"

strTable			= "vwPI_List"
strPK				= "PI_Code"
strSelect			= ""
strSelect			= strSelect		& "PI_Code,PI_Issued_Date,Parts_P_P_No,PI_To_Date,PI_In_Date,Partner_P_Name,PI_Price,PI_Qty,Sum_Price_Qty,PI_Remark,PI_State,PI_Payment_Type"
...
if Request("s_edit_mode_yn") = "" then
	...
	strEdit	= ",,pno,dt1,dt1,"&strEdit_Partner&",txt,txt,txt,txt,"&BasicDataPartsIncomingState&","&BasicDataPartnerPaymentType
end if
strPopup			= ",,,,,,,,,,,,,"
strDown				= ",,,,,,,,,,,,,"
strAlign			= "Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center,Center"
'----------------------------------------------------------------------------------

'3/9
'----------------------------------------------------------------------------------

if Len(Request("s_PI_Issued_Date")) = "22" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PI_Issued_Date between ''"&Left(Request("s_PI_Issued_Date"),10)&"'' and ''"&Right(Request("s_PI_Issued_Date"),10)&"''"
end if

if Len(Request("s_PI_In_Date")) = "22" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PI_In_Date between ''"&Left(Request("s_PI_In_Date"),10)&"'' and ''"&Right(Request("s_PI_In_Date"),10)&"''"
end if

if Len(Request("s_PI_To_Date")) = "22" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PI_To_Date between ''"&Left(Request("s_PI_To_Date"),10)&"'' and ''"&Right(Request("s_PI_To_Date"),10)&"''"
end if
...
if Request("s_PI_Price_up") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PI_Price >= "&Request("s_PI_Price_up")
end If
if Request("s_PI_Price_down") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PI_Price <= "&Request("s_PI_Price_down")
end If
if Request("s_PI_Qty_up") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PI_Qty >= "&Request("s_PI_Qty_up")
end If
if Request("s_PI_Qty_down") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PI_Qty <= "&Request("s_PI_Qty_down")
end If

if Request("s_PI_State") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PI_State = ''"&Request("s_PI_State")&"''"
end If
...
if Request("s_PI_Payment_Type") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PI_Payment_Type = ''"&Request("s_PI_Payment_Type")&"''"
end If


if S_Order_By_1 & S_Order_By_2 = "" then
	S_Order_By_1 	= "PI_Code"
	S_Order_By_2 	= "desc"
end if
...
'4/9
'----------------------------------------------------------------------------------	
strColumn		= "s_PI_Issued_Date,s_PI_To_Date,s_PI_In_Date,s_Parts_P_P_No|/|s_PI_Price_up,s_PI_Qty_up,s_Partner_P_Name,s_PI_State|/|s_PI_Price_down,s_PI_Qty_down,s_PI_Payment_Type,s_edit_mode_yn"
strName			= "발주일,납기일,입고일,파트넘버|/|단가(이상),수량(이상),거래처,상태|/|단가(이하),수량(이하),거래구분,수정모드"
strType			= "dt2,dt2,dt2,txt|/|num,num,"&strEdit_Partner&","&BasicDataPartsIncomingState&"|/|num,num,"&BasicDataPartnerPaymentType&",chk"
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------	
	strReg	= ",dt1"&date()&",pno,dt1,,"&strEdit_Partner&",txt,txt,txt,txt,"&BasicDataPartsIncomingState&","&BasicDataPartnerPaymentType
'----------------------------------------------------------------------------------	
	...
	call Parts_Guide()
	call inc_Common_List_Reg_Form_Parts_Incoming_Multi(URL_Reg, Colspan, strRequestQueryString, strSelect, arrRecordSet, strWidth, strReg, strAlign, strWidth_Total)
end if
%>
...
call inc_Common_List_Parts_Incoming(strID, strID_Pos, S_PageNo, URL_This, URL_View, URL_Action, arrSelectName, strSelect, arrRecordSet, TotalRecordCount, Colspan, strRequestQueryString, S_Order_By_1, S_Order_By_2, strPopup, strDown, strWidth, strEdit, strAlign, strWidth_Total)
%>

<script language="javascript">
function cal_Sum_Price_Qty(strFormname,strObjIdx)
{
	objPI_Price			= eval(strFormname+".PI_Price["+strObjIdx+"]");
	objPI_Qty			= eval(strFormname+".PI_Qty["+strObjIdx+"]");
	objSum_Price_Qty	= eval(strFormname+".Sum_Price_Qty["+strObjIdx+"]");
	
	if(IsFloat(objPI_Price.value) && IsFloat(objPI_Qty.value))
	{
		objSum_Price_Qty.value = Math.round(parseFloat(objPI_Price.value) * parseFloat(objPI_Qty.value) * 100)/100;
	}
	else
	{
...
function Reset_Form(strFormname,strObjIdx)
{
	objPI_Price			= eval(strFormname+".PI_Price["+strObjIdx+"]");
	objPI_Qty			= eval(strFormname+".PI_Qty["+strObjIdx+"]");
	objPI_Payment_Type	= eval(strFormname+".PI_Payment_Type["+strObjIdx+"]");
	objSum_Price_Qty	= eval(strFormname+".Sum_Price_Qty["+strObjIdx+"]");

	objPI_Price.value			= "";
	objPI_Qty.value				= "";
	objPI_Payment_Type.value	= "";
	objSum_Price_Qty.value		= "";
}
...
function getParts_Info(strFormname,strObjIdx)
{
	...
	{
		ifrmParts_Info.location.href="/parts_incoming/inc_parts_info.asp?strFormname="+strFormname+"&s_P_P_No=" + objP_P_No.value + "&strObjIdx="+strObjIdx;		
	}
}
...
function getParts_Partner_Info(strFormname,strObjIdx)
{
	...
	if (objP_P_No.value.length != "")
	{
		ifrmParts_Info.location.href="/parts_incoming/inc_parts_info.asp?strFormname="+strFormname+"&s_P_P_No=" + objP_P_No.value + "&s_Partner_P_Name=" + encodeURL(objPartner_P_Name.value) + "&strObjIdx="+strObjIdx;		
	}
}

<%
if Request("strChecked_Value") <> "" then
%>

RegForm_Toggle();

<%
	dim arrChecked_Value
	arrChecked_Value = split(Request("strChecked_Value"),",")

	set RS1 = Server.CreateObject("ADODB.RecordSet")
	for CNT1 = 0 to ubound(arrChecked_Value) - 1
		SQL = "select P_P_No from tbParts where P_Code = '"&arrChecked_Value(CNT1)&"'"
		RS1.Open SQL,sys_DBCon
		if not(RS1.Eof or RS1.Bof) then
%>
Fill_Form('frmCommonListReg','Parts_P_P_No[<%=CNT1%>]','<%=RS1("P_P_No")%>');
getParts_Info('frmCommonListReg','<%=CNT1%>');
<%
		end if
		RS1.Close
	next
...
call Common_Display_Summary("발주금액 총계 : |Sum_String|", SQL, "Y", strWidth_Total)
'----------------------------------------------------------------------------------
%>
...
'----------------------------------------------------------------------------------
%>
	//var strError = List_Reg_Validater_Multi('PI_Issued_Date,Parts_P_P_No,PI_Price,PI_Qty','발주일,파트넘버,단가,주문수량','txt,txt,num,num');
	var strError = "";
<%													
'----------------------------------------------------------------------------------
%>
...
'----------------------------------------------------------------------------------
%>
	//var strError = List_Validater('PI_Issued_Date,Parts_P_P_No,PI_Price,PI_Qty','발주일,파트넘버,단가,주문수량','txt,txt,num,num');
	var strError = "";
<%
'----------------------------------------------------------------------------------
%>
...
function fPrint()
{
	...
	{
		window.open("pi_print.asp?strChecked_Value="+encodeURL(strChecked_Value),"PartsOrderSheet","height="+screen.height+",width="+screen.width+",status=yes,toolbar=yes,location=yes,directories=yes,location=yes,menubar=yes,resizable=yes,scrollbars=yes,titlebar=yes")
	}
}
...
function fMail()
{
	...
	{
		window.open("pi_mail_form.asp?strChecked_Value="+encodeURL(strChecked_Value),"PartsOrderMail","height=100,width=300,status=no,toolbar=no,location=no,directories=no,location=no,menubar=no,resizable=no,scrollbars=no,titlebar=no")
	}
}
...
<tr>
	<td align=center>
		...
		<tr>
<%
if Request("s_edit_mode_yn") <> "" and instr(admin_pi_list,"-"&Request("Admin")("M_ID")&"-") > 0 then
%>
			<td width=5px></td>
...
%>
<%
if Reg_Form_YN = "Y" and instr(admin_pi_list,"-"&Request("Admin")("M_ID")&"-") > 0 then
%>		
			<td width=5px></td>
			<td width=77px>
...
%>
			<td width=5px></td>
			<td width=77px><%=Make_BTN("발주서출력","javascript:fPrint()","")%></td>
			<td width=5px></td>
			<td width=77px><%=Make_BTN("메일발주","javascript:fMail()","")%></td>
			<td width=5px></td>
<%
end if
%>
...
<%
sub inc_Common_List_Reg_Form_Parts_Incoming_Multi(URL_Reg, Colspan, strRequestQueryString, strSelect, arrRecordSet, strWidth, strReg, strAlign, strWidth_Total)
	dim CNT0
	dim CNT1
	dim CNT2
	dim CNT3
...
<%
		...
		for CNT1 = 0 to ubound(arrRecordSet,1)
			if arrEdit(CNT1) <> "" then				
				...
				case "txt"
%>
			<td><input type="text" size=10 name="<%=arrSelect(CNT1)%>"<%if instr("-Sum_Price_Qty-","-"&arrSelect(CNT1)&"-") > 0 then%> readonly<%end if%> <%if instr("-PI_Qty-PI_Price-","-"&arrSelect(CNT1)&"-") > 0 then%> onkeyup="javascript:cal_Sum_Price_Qty('frmCommonListReg',<%=CNT0%>);"<%end if%> value="" class=input style="width:100%;text-align:<%=arrAlign(CNT1)%>;height:<%=strHeight-1%>px;padding-top:<%=nPaddingTop%>px; "></td>
<%
				case "mem"
%>
...
<%
				if arrSelect(CNT1) = "PI_State" then
%>
				<option value="발주준비">발주준비</option>
<%				
				else
%>
...
<%
sub inc_Common_List_Parts_Incoming(strID, strID_Pos, S_PageNo, URL_This,URL_View, URL_Action, arrSelectName, strSelect, arrRecordSet, TotalRecordCount, Colspan, strRequestQueryString, S_Order_By_1, S_Order_By_2, strPopup, strDown, strWidth, strEdit, strAlign, strWidth_Total)
	dim CNT1
	dim CNT2
	dim CNT3
