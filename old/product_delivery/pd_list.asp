...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "번호,입고일,파트넘버,제번,수량,통화,단가,입고금액,출발일,구분"

strWidth			= ""
strWidth			= strWidth		& "70,100,110,120,70,70,100,100,90,70"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/product_delivery/pd_list.asp"
URL_View			= "/product_delivery/pd_edit_form.asp"
URL_Action			= "/product_delivery/pd_list_action.asp"
URL_Reg				= "/product_delivery/pd_reg_action.asp"

strTable			= "vwPD_List"
strPK				= "pd_code"
strSelect			= ""
strSelect			= strSelect		& "PD_Code,PD_Receiving_Date,BOM_Sub_BS_D_No,PD_Work_Order,PD_Receiving_Qty,PD_Currency,PD_Unit_Price,PD_Sum_Price,PD_Departure_Date,PD_Market"
...
'3/9
'----------------------------------------------------------------------------------
if Request("s_PD_Receiving_Date") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PD_Receiving_Date = ''"&Request("s_PD_Receiving_Date")&"''"
end If

if Len(Request("s_PD_Receiving_Date2")) = "22" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PD_Receiving_Date between ''"&Left(Request("s_PD_Receiving_Date2"),10)&"'' and ''"&Right(Request("s_PD_Receiving_Date2"),10)&"''"
end if
...
if Request("s_PD_Work_Order") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	
	if instr(Request("s_PD_Work_Order"),",") > 0 then
		strWhere = strWhere & "left(PD_Work_Order,8) in (''"&replace(replace(Request("s_PD_Work_Order")," ",""),",","'',''")&"'')"
	else
		strWhere = strWhere & "PD_Work_Order like ''%"&Request("s_PD_Work_Order")&"%''"
	end if
end If

if Request("s_PD_Market") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "PD_Market = ''"&Request("s_PD_Market")&"''"
end If


if S_Order_By_1 & S_Order_By_2 = "" then
	S_Order_By_1 	= "PD_Receiving_Date"
	S_Order_By_2 	= "desc"
	S_Order_By_3 	= "pd_code"
	S_Order_By_4 	= "desc"
end if
...
'4/9
'----------------------------------------------------------------------------------	
strColumn		= "s_PD_Receiving_Date,s_PD_Receiving_Date2,s_BOM_Sub_BS_D_No,s_PD_Work_Order,s_PD_Market"
strName			= "입고일,입고기간,파트넘버,워크오더,내수/수출"
strType			= "dt1,dt2,txt,txt,slt>내수:내수;수출:수출"
'----------------------------------------------------------------------------------
