...
arrPO_Issued_Date		= split(Request("PO_Issued_Date")	,", ")
...
for CNT1 = 0 to ubound(arrID_All)
	...
	arrPO_Issued_Date(CNT1)		= trim(arrPO_Issued_Date(CNT1))
	...
	for CNT1 = 0 to ubound(arrID_All)
		...
		if strError_Temp = "" then
			if arrParts_P_P_No(CNT1) <> "" then
				...
				RS1.Open "tbParts_Outgoing",sys_DBConString,3,2,2
				with RS1
					.AddNew

					.Fields("Parts_P_P_No")		= arrParts_P_P_No(CNT1)
					if arrPO_Issued_Date(CNT1) <> "" then
						.Fields("PO_Issued_Date")	= arrPO_Issued_Date(CNT1)
					end if
				...
				SQL = "update tbPartner set P_Payment_Type = '"&arrPO_Payment_Type(CNT1)&"' where P_Name='"&arrPartner_P_Name(CNT1)&"'"
