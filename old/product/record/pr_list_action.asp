...
if strError = "" then	
	...
	for CNT1 = 0 to ubound(arrID_All)
		...
		if strError_Temp = "" then
			...
			SQL = "select top 1 PR_WorkType,BOM_Sub_BS_D_No,PR_Process,PR_Amount from tbProcess_Record where PR_Code = '"&arrID_All(CNT1)&"'"
			...
			if not(RS1.Eof or RS1.Bof) then
				...
				if BOM_Sub_BS_D_No = arrBOM_Sub_BS_D_No(CNT1) then	'동일 도번
					...
					elseif PR_WorkType = "작업" and newPR_WorkType = "작업" then '계속 작업 -> 이전수량을 차감, 새로운 수량을 증가
						...
						call Process_Qty_BOM_Sub_Plus(BOM_Sub_BS_D_No,PR_Process,int(arrPR_Amount(CNT1)) - int(PR_Amount))
						
						'입력된 실적에 해당하는 모델파트넘버의 이전공정 재고를 -시킴
						call Process_Qty_BOM_Sub_Before_Minus(BOM_Sub_BS_D_No,PR_Process,int(arrPR_Amount(CNT1)) - int(PR_Amount))
					...
					elseif PR_WorkType <> "작업" and newPR_WorkType = "작업" then '작업이 아니다가 작업으로 변환 -> 새로운 수량을 증가
						'입력된 실적에 해당하는 모델파트넘버의 해당공정 재고를 +시킴
						call Process_Qty_BOM_Sub_Plus(arrBOM_Sub_BS_D_No(CNT1),PR_Process,arrPR_Amount(CNT1))
						
						'입력된 실적에 해당하는 모델파트넘버의 이전공정 재고를 -시킴
						call Process_Qty_BOM_Sub_Before_Minus(arrBOM_Sub_BS_D_No(CNT1),PR_Process,arrPR_Amount(CNT1))
