...
strSelectName		= strSelectName & "번호,작업날짜,제번,라인,직접인원,간접인원,파트넘버,계획수량,시작시각,종료시각,비고,삭제"
strWidth			= ""
strWidth			= strWidth		& "50,80,80,70,70,70,90,70,80,80,150,60"
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/process_record/pr_plan_list.asp"
URL_View			= "/process_record/pr_plan_delete_action.asp"
URL_Action			= "/process_record/pr_plan_list_action.asp"
URL_Reg				= "/process_record/pr_plan_reg_action.asp"

strTable			= "vwPR_Plan_List"
strPK				= "PR_Code"
strSelect			= ""
strSelect			= strSelect		& "PR_Code,PR_Work_Date,PR_Work_Order,PR_Line,PR_Worker_CNT,PR_Supporter_CNT,BOM_Sub_BS_D_No,PR_Plan_Amount,PR_Plan_Start_Time,PR_Plan_End_Time,PR_Memo"
...
if Request("s_edit_mode_yn") = "" then
	...
	select case S_PR_Process
	case "IMD"	
		strEdit	= ",dt1"&date()&",w/o,"&BasicDataIMDLine&",txt,txt,dn1,txt,txt,txt,txt"	
	case "SMD"
		strEdit	= ",dt1"&date()&",w/o,"&BasicDataSMDLine&",txt,txt,dn2,txt,txt,txt,txt"
	case "MAN"
		strEdit	= ",dt1"&date()&",w/o,"&BasicDataMANLine&",txt,txt,dn2,txt,txt,txt,txt"
	case "ASM"
		strEdit	= ",dt1"&date()&",w/o,"&BasicDataASMLine&",txt,txt,dn2,txt,txt,txt,txt"
	case "DLV"
		strEdit	= ",dt1"&date()&",w/o,"&BasicDataDLVLine&",txt,txt,dn2,txt,txt,txt,txt"
	end select
end if
strPopup			= ",,,,,,,,,,,,,,,"
strDown				= ",,,,,,,,,,,,,,,"
...
End If
strWhere = strWhere & "PR_WorkType = ''작업''"
...
if Request("s_pr_work_order") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "pr_work_order = ''"&Request("s_pr_work_order")&"''"
end If
...
if S_Order_By_1 & S_Order_By_2 = "" then
	...
	S_Order_By_2 	= "asc"
	S_Order_By_3 	= "pr_plan_start_time"
	S_Order_By_4 	= "asc"
end if
...
'4/9
'----------------------------------------------------------------------------------	
strColumn		= "s_pr_process,s_pr_work_order,s_BOM_Sub_BS_D_No,s_pr_work_date,s_pr_line,s_edit_mode_yn"
strName			= "공정,제번,파트넘버,작업일,라인,수정모드"
select case S_PR_Process
case "IMD"
	strType	= BasicDataProcess&",txt,dn1,dt1"&date()&","&BasicDataIMDLine&",chk"
case "SMD"
	strType	= BasicDataProcess&",txt,dn2,dt1"&date()&","&BasicDataSMDLine&",chk"
case "MAN"
	strType	= BasicDataProcess&",txt,dn2,dt1"&date()&","&BasicDataMANLine&",chk"
case "ASM"
	strType	= BasicDataProcess&",txt,dn2,dt1"&date()&","&BasicDataASMLine&",chk"
case "DLV"
	strType	= BasicDataProcess&",txt,dn2,dt1"&date()&","&BasicDataDLVLine&",chk"
end select
'----------------------------------------------------------------------------------
...
select case S_PR_Process
case "IMD"
	strReg	= ",dt1"&date()&",w/o,"&BasicDataIMDLine&",txt1,txt0,dn1,txt,txt,txt,txt"
case "SMD"
	strReg	= ",dt1"&date()&",w/o,"&BasicDataSMDLine&",txt1,txt0,dn2,txt,txt,txt,txt"
case "MAN"
	strReg	= ",dt1"&date()&",w/o,"&BasicDataMANLine&",txt,txt,dn2,txt,txt,txt,txt"
case "ASM"
	strReg	= ",dt1"&date()&",w/o,"&BasicDataASMLine&",txt,txt,dn2,txt,txt,txt,txt"
case "DLV"
	strReg	= ",dt1"&date()&",w/o,"&BasicDataDLVLine&",txt1,txt0,dn2,txt,txt,txt,txt"
end select
...
if instr("-DLV-",S_PR_Process) > 0 then
	SQL = "select sum(PR_Plan_Amount) from "&strTable&" "
	
	if trim(strWhere) <> "" then
		SQL = SQL & " where " & strWhere
	end if
	
	call Common_Display_Summary("납품계획수량 총계 : |Sum_String|", SQL, "N", strWidth_Total)
else
	SQL = "select sum(PR_Plan_Amount) from "&strTable&" "
