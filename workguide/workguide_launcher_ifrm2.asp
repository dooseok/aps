...
workguide_launcher_ifrm2
<%
'변수 선언!

'231012 PCBA 7번 모니터 이후로, 수동으로 모델 체인지 되도록 코드수정
'구현 설명
'이 파일이 실행될 때, tbWorkGuide 테이블의 WG_PartNo_6 칼럼에 6번 모니터의 파트넘버를 기록.
'PCBA키훅에서 F10을 누를 때, tbWorkGuide_7mon에 tbWorkGuide의 WG_PartNo_6 값을, PartNo_7로 insert. (admin_2080\status_monitor\barcode2db_process\barcode2db_process.asp에서 코드구현)
'7번모니터 이후는, tbWorkGuide_7mon에서 파트넘버를 조회해서 가져옴
'테스트는 keyhook 프로그램을 통해서 할 수 있으며, kr.msekorea.com/test3.asp?line=PCBA5 를 통해서 도 할 수 있다.
...
'7mon---
CNT2 = 6
SQL = "select top 50 PartNo_7, PartNo_7_StartTime from tbWorkGuide_7mon where "
SQL = SQL & "PRD_Line='"&s_PRD_Line&"' "
SQL = SQL & "order by WG7_Code desc "
'response.write SQL
'현황판 변경 후------------------------------]
RS1.Open SQL,sys_DBCon
ResultYN="N"
...
if ResultYN = "Y" then
	for CNT1 = 0 to ubound(arrRecordSet,2)
		dbtime = arrRecordSet(1,CNT1)
		arrHHMMSS = split(formatDateTime(dateadd("s",-1*(arrWG_MCDelay(CNT2)-arrWG_MCDelay(6)),Time), vbLongTime),":")
		AMPM = left(arrHHMMSS(0),2)
		arrHHMMSS(0) = replace(arrHHMMSS(0),AMPM&" ","")
...
for CNT1 = 0 to 15
	response.write CNT1&":"&arrPRD_PartNo(CNT1) &"_"&arrWG_MCDelay(CNT1)&"<br>"
next
...
'7mon---
if instr(s_PRD_Line,"PCBA") > 0 then
	SQL = "update tbWorkGuide set WG_PartNo_6 = '"&arrPRD_PartNo(5)&"' where PRD_Line = '"&s_PRD_Line&"' and WG_Pos=6"
	sys_DBCon.execute(SQL)
end if
'---7mon
'공정별 루프
...
{
	location.href="workguide_launcher_ifrm2.asp?s_PRD_Line=<%=s_PRD_Line%>&strIDX=<%=server.urlencode(strIDX)%>&strSlideCNT=<%=server.urlencode(strSlideCNT)%>&strPrePartNo=<%=server.urlencode(strPrePartNo)%>&";
}
