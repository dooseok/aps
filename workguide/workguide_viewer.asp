...
var element = document.getElementById("imgWorkGuide");
addEvent("load",element,function(){
    var nWidth = element.width;
    if (nWidth > 1) {
	    var nHeight = element.height;
	    element.height = <%=WG_ResY-78%>
	    element.width = parseInt(<%=WG_ResY-78%> * parseFloat( nWidth / nHeight ));
	}
})
</script>
<%
...
'[현황판 변경 후------------------------------
	SQL = ""
	SQL = SQL & "select top 3000 SML_PartNo from tblStatus_Monitor_Line where "
...
{
	location.href='workguide_viewer.asp?bReload_YN=Y&WG_SlideDelay=<%=WG_SlideDelay%>&WG_SlideDelay_Main=<%=WG_SlideDelay_Main%>&s_PRD_Line=<%=s_PRD_Line%>&WG_Auto_YN=<%=WG_Auto_YN%>&WG_Pos=<%=WG_Pos%>&WG_ResX=<%=WG_ResX%>&WG_ResY=<%=WG_ResY%>&strCurrentPartNo='+strCurrentPartNo;
}
...
function selectPartNo(strPartNo)
{
	...
	ifrmViewer_ifrm.location.href="workguide_viewer_ifrm.asp?strPartNo="+strPartNo+"&s_PRD_Line=<%=s_PRD_Line%>&WG_Pos=<%=WG_Pos%>&WG_ResX=<%=WG_ResX%>&WG_ResY=<%=WG_ResY%>&WG_SlideDelay=<%=WG_SlideDelay%>&WG_SlideDelay_Main=<%=WG_SlideDelay_Main%>";
	closeSelector();
}
