...
'----------------------------------------------------------------------------------
strSelectName		= ""
strSelectName		= strSelectName & "체크,번호,파트넘버,공정,파트이름,스펙(요약),스펙,메이커,현재재고,안전재고,사급가,LG가,실구입가,거래처,동일파트,대체출고"

strWidth			= ""
strWidth			= strWidth		& "30,40,100,60,120,160,200,100,65,65,75,75,75,120,75,100"
'----------------------------------------------------------------------------------
...
'2/9
'----------------------------------------------------------------------------------
URL_This			= "/parts/p_list.asp"
URL_View			= "/parts/p_edit_form.asp"
URL_Action			= "/parts/p_list_action.asp"
URL_Reg				= "/parts/p_reg_action.asp"

strTable			= "vwP_List"
strPK				= "p_code"
strSelect			= ""
strSelect			= strSelect		& "P_Code,P_P_No,P_Work_Type,P_Desc,P_Spec_Short,P_Spec,P_Maker,P_Qty,P_Safe_Qty,P_CO_Price,P_LGE_Price,P_MSE_Price,Partner_P_Name,P_Same_Code,P_Real_Demand_P_P_No"
...
SQL ="select P_Name from tbPartner order by P_Name asc"
...
if Request("s_edit_mode_yn") = "" then
	...
	strEdit	= ",mem,"&BasicDataPartsType&",mem,mem,mem,mem,,num,,mny,mny,"&"slt>:미정;"&replace(strEdit_Partner,"slt>","")&",num,pno"
end if
strPopup			= ",,,,,,,,,,,,,,,"
strDown				= ",,,,,,,,,,,,,,,"
strAlign			= ""
strAlign			= strAlign		& "Center,Center,Center,Center,Center,Center,Center,right,right,right,"
strAlign			= strAlign		& "right,Right,Center,Center,Center"
'----------------------------------------------------------------------------------
...
if Request("s_p_spec") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "(p_spec like ''%"&Request("s_p_spec")&"%'' or p_spec_short like ''%"&Request("s_p_spec")&"%'')"
end If

if Request("s_p_maker") <> "" then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "p_maker like ''%"&Request("s_p_maker")&"%''"
end If
...
if Request("s_None_Work_Type") <> "" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "(P_Work_Type ='''' or P_Work_Type is null)"
end if

if Request("s_same_code") <> "" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "(convert(int,P_same_code) > 0)"
end if

if Request("s_real_demand") <> "" Then
	If Trim(strWhere) <> "" Then
		strWhere = strWhere & " and "
	End If
	strWhere = strWhere & "(P_Real_Demand_P_P_No <> '''' and P_Real_Demand_P_P_No is not null)"
end if
...
'----------------------------------------------------------------------------------	
Call BOMSub_Guide
strColumn		= "s_bom_sub_bs_d_no,s_p_p_no,s_p_desc,s_p_spec,s_p_maker|/|s_P_Qty_up,s_P_Qty_down,s_partner_p_name,s_safe_qty,s_None_Work_Type|/|s_same_code,s_real_demand,s_edit_mode_yn"
strName			= "모델파트넘버,부품파트넘버,파트이름,스펙,메이커|/|재고량(이상),재고량(이하),거래처,안전재고이하,공정미등록|/|동일파트항목,대체출고항목,수정모드"
strType			= "dn2,txt,txt,txt,txt|/|num,num,"&strEdit_Partner&",chk,chk|/|chk,chk,chk"
'----------------------------------------------------------------------------------
...
'6/9
'----------------------------------------------------------------------------------	
	strReg	= ",mem,"&BasicDataPartsType&",mem,mem,mem,mem,,num,,mny,mny,"&"slt>:미정;"&replace(strEdit_Partner,"slt>","")&",num,pno"
'----------------------------------------------------------------------------------	
...
'6.5/9
'----------------------------------------------------------------------------------
SQL = "select sum(P_Qty*P_MSE_Price) from "&strTable&" "

if trim(strWhere) <> "" then
	SQL = SQL & " where " & strWhere
end if

call Common_Display_Summary("재고금액 총계 : |Sum_String|", SQL, "Y", strWidth_Total)
'----------------------------------------------------------------------------------
%>
...
<tr>
	<td align=center>
		...
		<tr>
<%
if Request("s_edit_mode_yn") <> "" and instr(admin_p_list,"-"&gM_ID&"-") > 0 then
%>
			<td width=5px></td>
...
%>
<%
if Reg_Form_YN = "Y" and instr(admin_p_list,"-"&gM_ID&"-") > 0 then
%>		
			<td width=5px></td>
			<td width=77px>
